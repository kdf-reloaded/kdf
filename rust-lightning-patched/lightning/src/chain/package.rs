...
use chain::chaininterface::{ConfirmationTarget, FeeEstimator, MIN_RELAY_FEE_SAT_PER_1000_WEIGHT};
...
impl HolderFundingOutput {
    pub(crate) fn build(funding_redeemscript: Script) -> Self {
        HolderFundingOutput { funding_redeemscript }
    }
}
...
impl PackageSolvingData {
    ...
    fn finalize_input<Signer: Sign>(
        ...
        match self {
            PackageSolvingData::RevokedOutput(ref outp) => {
                if let Ok(chan_keys) = TxCreationKeys::derive_new(
            ...
            PackageSolvingData::RevokedHTLCOutput(ref outp) => {
                if let Ok(chan_keys) = TxCreationKeys::derive_new(
                    ...
                    if let Ok(sig) = onchain_handler.signer.sign_justice_revoked_htlc(
                        ...
                        bumped_tx.input[i]
                            .witness
                            .push(chan_keys.revocation_key.clone().serialize().to_vec());
                        bumped_tx.input[i].witness.push(witness_script.clone().into_bytes());
                    } else {
                        return false;
                    }
    ...
    fn get_finalized_tx<Signer: Sign>(
        &self,
    ...
    fn absolute_tx_timelock(&self, output_conf_height: u32) -> u32 {
        ...
        let absolute_timelock = match self {
            ...
            PackageSolvingData::CounterpartyReceivedHTLCOutput(ref outp) => {
                cmp::max(outp.htlc.cltv_expiry, output_conf_height + 1)
            },
...
impl PackageTemplate {
    ...
    pub(crate) fn package_timelock(&self) -> u32 {
        self.inputs
            .iter()
            .map(|(_, outp)| outp.absolute_tx_timelock(self.height_original))
            .max()
    ...
    pub(crate) fn finalize_package<L: Deref, Signer: Sign>(
        &self,
        onchain_handler: &mut OnchainTxHandler<Signer>,
        value: u64,
        ...
        L::Target: Logger,
    {
        match self.malleability {
            ...
            PackageMalleability::Untractable => {
                debug_assert_eq!(
                    value, 0,
                    "value is ignored for non-malleable packages, should be zero to ensure callsites are correct"
                );
    ...
    L::Target: Logger,
{
    ...
    let new_fee = if let Some((new_fee, _)) =
        compute_fee_from_spent_amounts(input_amounts, predicted_weight, fee_estimator, logger)
    {
...
mod tests {
    use chain::package::{
        weight_offered_htlc, weight_received_htlc, CounterpartyOfferedHTLCOutput, CounterpartyReceivedHTLCOutput,
        HolderHTLCOutput, PackageSolvingData, PackageTemplate, RevokedOutput, WEIGHT_REVOKED_OUTPUT,
    };
    use chain::Txid;
    ...
    macro_rules! dumb_counterparty_output {
        ($secp_ctx: expr, $amt: expr) => {{
            ...
            PackageSolvingData::CounterpartyReceivedHTLCOutput(CounterpartyReceivedHTLCOutput::build(
                dumb_point, dumb_point, dumb_point, htlc,
            ))
        }};
    }
    ...
    macro_rules! dumb_counterparty_offered_output {
        ($secp_ctx: expr, $amt: expr) => {{
            ...
            PackageSolvingData::CounterpartyOfferedHTLCOutput(CounterpartyOfferedHTLCOutput::build(
                dumb_point, dumb_point, dumb_point, preimage, htlc,
            ))
        }};
    }
    ...
    fn test_package_weight() {
...

        {
            ...
            for &opt_anchors in [false, true].iter() {
                assert_eq!(
                    package.package_weight(&Script::new(), opt_anchors),
                    weight_sans_output + WEIGHT_REVOKED_OUTPUT as usize
                );
            }
        }

        {
            ...
            for &opt_anchors in [false, true].iter() {
                assert_eq!(
                    package.package_weight(&Script::new(), opt_anchors),
                    weight_sans_output + weight_received_htlc(opt_anchors) as usize
                );
            }
        }

        {
            ...
            for &opt_anchors in [false, true].iter() {
                assert_eq!(
                    package.package_weight(&Script::new(), opt_anchors),
                    weight_sans_output + weight_offered_htlc(opt_anchors) as usize
                );
            }
        }
    }
