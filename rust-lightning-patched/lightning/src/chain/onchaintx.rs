...
use util::ser::{MaybeReadable, Readable, ReadableArgs, VecWriter, Writeable, Writer};
...
impl<ChannelSigner: Sign> OnchainTxHandler<ChannelSigner> {
    ...
    pub(crate) fn get_prev_holder_commitment_to_self_value(&self) -> Option<u64> {
        self.prev_holder_commitment
            .as_ref()
            .map(|commitment| commitment.to_broadcaster_value_sat())
    }
    ...
    fn generate_claim_tx<F: Deref, L: Deref>(
        &mut self,
        cur_height: u32,
        cached_request: &PackageTemplate,
        fee_estimator: &F,
        logger: &L,
    ...
    where
        ...
        L::Target: Logger,
    {
        if cached_request.outpoints().len() == 0 {
            return None;
        ...
        if cached_request.is_malleable() {
            let predicted_weight = cached_request.package_weight(
                &self.destination_script,
                self.channel_transaction_parameters.opt_anchors.is_some(),
            );
            if let Some((output_value, new_feerate)) = cached_request.compute_package_output(
                predicted_weight,
                self.destination_script.dust_value().as_sat(),
                fee_estimator,
                logger,
            ) {
                ...
                let transaction = cached_request
                    .finalize_package(self, output_value, self.destination_script.clone(), logger)
                    .unwrap();
                log_trace!(
                    logger,
                    "...with timer {} and feerate {}",
                    new_timer.unwrap(),
                    new_feerate
                );
    ...
    pub(crate) fn update_claims_view<B: Deref, F: Deref, L: Deref>(
        &mut self,
        ...
        L::Target: Logger,
    {
        log_debug!(
            logger,
            "Updating claims view at height {} with {} matched transactions in block {} and {} claim requests",
            cur_height,
            txn_matched.len(),
            conf_height,
            requests.len()
        ...
        for req in requests {
            ...
            if let Some(_) = self.claimable_outpoints.get(req.outpoints()[0]) {
                log_info!(
                    logger,
                    "Ignoring second claim for outpoint {}:{}, already registered its claiming request",
                    req.outpoints()[0].txid,
                    req.outpoints()[0].vout
                );
            } else {
        ...
        for mut req in preprocessed_requests {
            if let Some((new_timer, new_feerate, tx)) =
                self.generate_claim_tx(cur_height, &req, &*fee_estimator, &*logger)
            {
                        ...
                        macro_rules! clean_claim_request_after_safety_delay {
                            () => {
                                let entry = OnchainEventEntry {
                                    txid: tx.txid(),
                                    height: conf_height,
                                    event: OnchainEvent::Claim {
                                        claim_request: first_claim_txid_height.0.clone(),
                                    },
                                };
        ...
        let onchain_events_awaiting_threshold_conf = self
            .onchain_events_awaiting_threshold_conf
            .drain(..)
            .collect::<Vec<_>>();
        ...
        for (first_claim_txid, request) in bump_candidates.iter() {
            if let Some((new_timer, new_feerate, bump_tx)) =
                self.generate_claim_tx(cur_height, &request, &*fee_estimator, &*logger)
            {
    ...
    pub(crate) fn block_disconnected<B: Deref, F: Deref, L: Deref>(
        &mut self,
        height: u32,
        broadcaster: B,
        fee_estimator: F,
        ...
        L::Target: Logger,
    {
        ...
        for (_, request) in bump_candidates.iter_mut() {
            if let Some((new_timer, new_feerate, bump_tx)) =
                self.generate_claim_tx(height, &request, &&*fee_estimator, &&*logger)
            {
                request.set_timer(new_timer);
                request.set_feerate(new_feerate);
    ...
    fn sign_latest_holder_htlcs(&mut self) {
        if self.holder_htlc_sigs.is_none() {
            let (_sig, sigs) = self
                .signer
                .sign_holder_commitment_and_htlcs(&self.holder_commitment, &self.secp_ctx)
    ...
    fn sign_prev_holder_htlcs(&mut self) {
        if self.prev_holder_htlc_sigs.is_none() {
            if let Some(ref holder_commitment) = self.prev_holder_commitment {
                let (_sig, sigs) = self
                    .signer
                    .sign_holder_commitment_and_htlcs(holder_commitment, &self.secp_ctx)
    ...
    fn extract_holder_sigs(
        holder_commitment: &HolderCommitmentTransaction,
        sigs: Vec<Signature>,
    ) -> Vec<Option<(usize, Signature)>> {
        let mut ret = Vec::new();
    ...
    pub(crate) fn get_fully_signed_copy_holder_tx(&mut self, funding_redeemscript: &Script) -> Transaction {
        let (sig, htlc_sigs) = self
            .signer
            .unsafe_sign_holder_commitment_and_htlcs(&self.holder_commitment, &self.secp_ctx)
            .expect("sign holder commitment");
        self.holder_htlc_sigs = Some(Self::extract_holder_sigs(&self.holder_commitment, htlc_sigs));
        self.holder_commitment.add_holder_sig(funding_redeemscript, sig)
    }
    ...
    pub(crate) fn get_fully_signed_htlc_tx(
        ...
        if commitment_txid == outp.txid {
            ...
            if let &Some(ref htlc_sigs) = &self.holder_htlc_sigs {
                ...
                htlc_tx = Some(trusted_tx.get_signed_htlc_tx(
    ...
    #[cfg(any(test, feature = "unsafe_revoked_tx_signing"))]
    pub(crate) fn unsafe_get_fully_signed_htlc_tx(
        &mut self,
        outp: &::bitcoin::OutPoint,
        preimage: &Option<PaymentPreimage>,
    ) -> Option<Transaction> {
