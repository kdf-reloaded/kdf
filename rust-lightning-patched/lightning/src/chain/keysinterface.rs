...
impl InMemorySigner {
    ...
    pub fn holder_selected_contest_delay(&self) -> u16 {
        self.get_channel_parameters().holder_selected_contest_delay
    }

    /// Whether the holder is the initiator
    /// Will panic if ready_channel wasn't called.
    pub fn is_outbound(&self) -> bool {
        self.get_channel_parameters().is_outbound_from_holder
    }

    /// Funding outpoint
    /// Will panic if ready_channel wasn't called.
    pub fn funding_outpoint(&self) -> &OutPoint {
        self.get_channel_parameters().funding_outpoint.as_ref().unwrap()
    }
    ...
    pub fn sign_counterparty_payment_input<C: Signing>(
        ...
        let witness_script = bitcoin::Address::p2pkh(
            &::bitcoin::PublicKey {
                compressed: true,
                key: remotepubkey,
            },
    ...
    pub fn sign_dynamic_p2wsh_input<C: Signing>(
        ...
        if spend_tx.input[input_idx].sequence != descriptor.to_self_delay as u32 {
            return Err(());
        }
...
impl BaseSign for InMemorySigner {
    fn get_per_commitment_point(&self, idx: u64, secp_ctx: &Secp256k1<secp256k1::All>) -> PublicKey {
        let commitment_secret =
            SecretKey::from_slice(&chan_utils::build_commitment_secret(&self.commitment_seed, idx)).unwrap();
    ...
    fn sign_counterparty_commitment(
        ...
        let channel_funding_redeemscript =
            make_funding_redeemscript(&funding_pubkey, &self.counterparty_pubkeys().funding_pubkey);
        ...
        for htlc in commitment_tx.htlcs() {
            ...
            let holder_htlc_key =
                chan_utils::derive_private_key(&secp_ctx, &keys.per_commitment_point, &self.htlc_base_key)
                    .map_err(|_| ())?;
    ...
    fn sign_holder_commitment_and_htlcs(
        ...
        let funding_redeemscript =
            make_funding_redeemscript(&funding_pubkey, &self.counterparty_pubkeys().funding_pubkey);
        let trusted_tx = commitment_tx.trust();
        let sig = trusted_tx.built_transaction().sign(
            &self.funding_key,
    ...
    fn sign_justice_revoked_output(
        ...
        let revocation_key =
            chan_utils::derive_private_revocation_key(&secp_ctx, &per_commitment_key, &self.revocation_base_key)
                .map_err(|_| ())?;
        ...
        .map_err(|_| ())?;
        let witness_script = {
            let counterparty_delayedpubkey = chan_utils::derive_public_key(
                &secp_ctx,
                &per_commitment_point,
                &self.counterparty_pubkeys().delayed_payment_basepoint,
            )
            .map_err(|_| ())?;
            chan_utils::get_revokeable_redeemscript(
                &revocation_pubkey,
        ...
        let sighash =
            hash_to_message!(&sighash_parts.signature_hash(input, &witness_script, amount, SigHashType::All)[..]);
    ...
    fn sign_justice_revoked_htlc(
        ...
        .map_err(|_| ())?;
        let witness_script = {
            let counterparty_htlcpubkey = chan_utils::derive_public_key(
                &secp_ctx,
                &per_commitment_point,
            ...
            let holder_htlcpubkey =
                chan_utils::derive_public_key(&secp_ctx, &per_commitment_point, &self.pubkeys().htlc_basepoint)
                    .map_err(|_| ())?;
    ...
    fn sign_counterparty_htlc_transaction(
        ...
        if let Ok(htlc_key) = chan_utils::derive_private_key(&secp_ctx, &per_commitment_point, &self.htlc_base_key) {
            let witness_script = if let Ok(revocation_pubkey) = chan_utils::derive_public_revocation_key(
                &secp_ctx,
                &per_commitment_point,
                &self.pubkeys().revocation_basepoint,
            ) {
                if let Ok(counterparty_htlcpubkey) = chan_utils::derive_public_key(
                    &secp_ctx,
                    &per_commitment_point,
                    &self.counterparty_pubkeys().htlc_basepoint,
                ) {
                    if let Ok(htlcpubkey) =
                        chan_utils::derive_public_key(&secp_ctx, &per_commitment_point, &self.pubkeys().htlc_basepoint)
                    {
                        chan_utils::get_htlc_redeemscript_with_explicit_keys(
                            &htlc,
    ...
    fn sign_channel_announcement(
        ...
        Ok((
            sign(secp_ctx, &msghash, &self.node_secret),
            sign(secp_ctx, &msghash, &self.funding_key),
        ))
    }
            ...
            match outp {
                SpendableOutputDescriptor::StaticPaymentOutput(descriptor) => {
                    ...
                    spend_tx.input[input_idx].witness = keys_cache
                        ...
                        .unwrap()
                        .0
                        .sign_counterparty_payment_input(&spend_tx, input_idx, &descriptor, &secp_ctx)?;
                },
                SpendableOutputDescriptor::DelayedPaymentOutput(descriptor) => {
                    if keys_cache.is_none() || keys_cache.as_ref().unwrap().1 != descriptor.channel_keys_id {
                        keys_cache = Some((
                    ...
                    spend_tx.input[input_idx].witness = keys_cache.as_ref().unwrap().0.sign_dynamic_p2wsh_input(
                        &spend_tx,
                        input_idx,
                        &descriptor,
                        &secp_ctx,
...
impl KeysInterface for KeysManager {
    ...
    fn sign_invoice(
        ...
        Ok(self
            .secp_ctx
            .sign_recoverable(&hash_to_message!(&Sha256::hash(&preimage)), &secret))
    }
}
...
impl PhantomKeysManager {
    ...
    pub fn new(seed: &[u8; 32], starting_time_secs: u64, starting_time_nanos: u32, cross_node_seed: &[u8; 32]) -> Self {
        ...
        let (inbound_key, phantom_key) =
            hkdf_extract_expand_twice(b"LDK Inbound and Phantom Payment Key Expansion", cross_node_seed);
        Self {
            inner,
