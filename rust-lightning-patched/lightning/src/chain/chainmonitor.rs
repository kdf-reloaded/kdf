...
use chain::channelmonitor::{
    Balance, ChannelMonitor, ChannelMonitorUpdate, MonitorEvent, TransactionOutputs, LATENCY_GRACE_PERIOD_BLOCKS,
};
...
impl MonitorUpdateId {
    pub(crate) fn from_monitor_update(update: &ChannelMonitorUpdate) -> Self {
        Self {
            contents: UpdateOrigin::OffChain(update.update_id),
        }
    }
    pub(crate) fn from_new_monitor<ChannelSigner: Sign>(monitor: &ChannelMonitor<ChannelSigner>) -> Self {
        Self {
            contents: UpdateOrigin::OffChain(monitor.get_latest_update_id()),
        }
    }
}
    ...
    P::Target: Persist<ChannelSigner>,
{
        ...
        let mut dependent_txdata = Vec::new();
        {
            ...
            for (funding_outpoint, monitor_state) in monitor_states.iter() {
                ...
                let mut txn_outputs;
                {
                    ...
                    match self
                        .persister
                        .update_persisted_channel(*funding_outpoint, &None, monitor, update_id)
                    {
                        Ok(()) => log_trace!(
                            self.logger,
                        ...
                        Err(ChannelMonitorUpdateErr::TemporaryFailure) => {
                            log_debug!(
                                self.logger,
                                "Channel Monitor sync for channel {} in progress, holding events until completion!",
                                log_funding_info!(monitor)
                            );
    ...
    pub fn channel_monitor_updated(
        ...
        match completed_update_id {
            MonitorUpdateId {
                contents: UpdateOrigin::OffChain(_),
            } => {
                ...
                self.pending_monitor_events
                    .lock()
                    .unwrap()
                    .push(MonitorEvent::UpdateCompleted {
                        funding_txo,
                        monitor_update_id: monitor_data.monitor.get_latest_update_id(),
                    });
            },
            MonitorUpdateId {
                contents: UpdateOrigin::ChainSync(_),
            } => {
                if !monitor_data.has_pending_chainsync_updates(&pending_monitor_updates) {
                    monitor_data
                        .last_chain_persist_height
                        .store(self.highest_chain_height.load(Ordering::Acquire), Ordering::Release);
    ...
    P::Target: Persist<ChannelSigner>,
{
    ...
    fn best_block_updated(&self, header: &BlockHeader, height: u32) {
        ...
        self.process_chain_data(header, Some(height), &[], |monitor, txdata| {
            ...
            monitor.best_block_updated(header, height, &*self.broadcaster, &*self.fee_estimator, &*self.logger)
        });
    }
...
impl<ChannelSigner: Sign, C: Deref, T: Deref, F: Deref, L: Deref, P: Deref> chain::Watch<ChannelSigner>
    for ChainMonitor<ChannelSigner, C, T, F, L, P>
where
    C::Target: chain::Filter,
    T::Target: BroadcasterInterface,
    ...
    P::Target: Persist<ChannelSigner>,
{
    ...
    fn watch_channel(
        ...
        let entry = match monitors.entry(funding_outpoint) {
            hash_map::Entry::Occupied(_) => {
                log_error!(
                    self.logger,
                    "Failed to add new channel data: channel monitor for given outpoint is already present"
                );
        ...
        let persist_res = self
            .persister
            .persist_new_channel(funding_outpoint, &monitor, update_id);
    ...
    fn update_channel(
        ...
        match monitors.get(&funding_txo) {
            ...
            Some(monitor_state) => {
                ...
                let persist_res =
                    self.persister
                        .update_persisted_channel(funding_txo, &Some(update), monitor, update_id);
    ...
    fn release_pending_monitor_events(&self) -> Vec<MonitorEvent> {
        ...
        for monitor_state in self.monitors.read().unwrap().values() {
            let is_pending_monitor_update =
                monitor_state.has_pending_chainsync_updates(&monitor_state.pending_monitor_updates.lock().unwrap());
                    ...
                    > self.highest_chain_height.load(Ordering::Acquire)
            {
                log_info!(
                    self.logger,
                    "A Channel Monitor sync is still in progress, refusing to provide monitor events!"
                );
            } else {
...
mod tests {
    ...
    use {expect_payment_path_successful, expect_payment_sent, expect_payment_sent_without_paths, get_event_msg};
    use {
        get_htlc_update_msgs, get_local_commitment_txn, get_revoke_commit_msgs, get_route_and_payment_hash,
        unwrap_send_err,
    };
    ...
    fn connect_block_checks_dependent_transactions() {
        ...
        nodes[1]
            .chain_source
            .expect(OnRegisterOutput {
                with: htlc_output,
                returns: Some((1, htlc_tx)),
            })
            .expect(OnRegisterOutput {
                with: to_local_output,
                returns: None,
            })
            .expect(OnRegisterOutput {
                with: htlc_timeout_output,
                returns: None,
            });
    ...
    fn test_async_ooo_offchain_updates() {
        ...
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_first_update);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_second_update);
        check_added_monitors!(nodes[1], 1);
    ...
    fn do_chainsync_pauses_events(block_timeout: bool) {
        ...
        if block_timeout {
            ...
            let persistences = chanmon_cfgs[0]
                .persister
                .chain_sync_monitor_persistences
                .lock()
                .unwrap()
