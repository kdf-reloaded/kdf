...
use chain::keysinterface::{
    DelayedPaymentOutputDescriptor, KeysInterface, Sign, SpendableOutputDescriptor, StaticPaymentOutputDescriptor,
};
use chain::onchaintx::OnchainTxHandler;
use chain::package::{
    CounterpartyOfferedHTLCOutput, CounterpartyReceivedHTLCOutput, HolderFundingOutput, HolderHTLCOutput,
...
use util::ser::{MaybeReadable, OptionDeserWrapper, Readable, ReadableArgs, Writeable, Writer, U48};
    ...
    fn confirmation_threshold(&self) -> u32 {
        ...
        match self.event {
            ...
            OnchainEvent::FundingSpendConfirmation {
                on_local_output_csv: Some(csv),
                ..
            }
            | OnchainEvent::HTLCSpendConfirmation {
                on_to_local_output_csv: Some(csv),
                ..
            } => {
                // A CSV'd transaction is confirmable in block (input height) + CSV delay, which means
                // it's broadcastable when we see the previous block.
...
impl<Signer: Sign> ChannelMonitor<Signer> {
    ...
    pub(crate) fn broadcast_latest_holder_commitment_txn<B: Deref, L: Deref>(&self, broadcaster: &B, logger: &L)
    where
        B::Target: BroadcasterInterface,
        L::Target: Logger,
    {
        self.inner
            .lock()
            .unwrap()
            .broadcast_latest_holder_commitment_txn(broadcaster, logger)
    }
        ...
        L::Target: Logger,
    {
        self.inner
            .lock()
            .unwrap()
            .unsafe_get_latest_holder_commitment_txn(logger)
    }
    ...
    pub fn get_claimable_balances(&self) -> Vec<Balance> {
        ...
        if let Some((txid, conf_thresh)) = funding_spend_pending {
            debug_assert!(
                us.funding_spend_confirmed.is_none(),
        ...
        if let Some(txid) = confirmed_txid {
                ...
                || Some(txid) == us.prev_counterparty_commitment_txid
            {
                walk_htlcs!(
                    false,
                    us.counterparty_claimable_outpoints
                        .get(&txid)
                        .unwrap()
                        .iter()
                        .map(|(a, _)| a)
...
impl<Signer: Sign> ChannelMonitorImpl<Signer> {
        ...
        L::Target: Logger,
    {
        ...
        if self.broadcasted_holder_revokable_script.is_some() {
            ...
            // transactions.
            let (claim_reqs, _) =
                self.get_broadcasted_holder_claims(&self.current_holder_commitment_tx, self.best_block.height());
            self.onchain_tx_handler.update_claims_view(
                &Vec::new(),
                claim_reqs,
                self.best_block.height(),
        ...
        for update in updates.updates.iter() {
            match update {
                ...
                ChannelMonitorUpdateStep::LatestCounterpartyCommitmentTXInfo {
                    ...
                    log_trace!(
                        logger,
                        "Updating ChannelMonitor with latest counterparty commitment transaction info"
                    );
                ...
                ChannelMonitorUpdateStep::ChannelForceClosed { should_broadcast } => {
                    ...
                    if *should_broadcast {
                        ...
                        log_info!(
                            logger,
                            "Channel off-chain state closed after we broadcasted our latest commitment transaction."
                        );
                    }
                },
    ...
    pub fn get_outputs_to_watch(&self) -> &HashMap<Txid, Vec<(u32, Script)>> {
        ...
        for (txid, _) in self.counterparty_commitment_txn_on_chain.iter() {
            self.outputs_to_watch
                .get(txid)
                .expect("Counterparty commitment txn which have been broadcast should have outputs registered");
        }
        &self.outputs_to_watch
    }
        ...
        L::Target: Logger,
    {
        ...
        if commitment_number >= self.get_min_seen_secret() {
                ...
                for (_, &(ref htlc, _)) in per_commitment_data.iter().enumerate() {
                    if let Some(transaction_output_index) = htlc.transaction_output_index {
                            ...
                            || tx.output[transaction_output_index as usize].value != htlc.amount_msat / 1000
                        {
                            ...
                            // Corrupted per_commitment_data, fuck this user
                        }
            ...
            fail_unbroadcast_htlcs!(
                ...
                per_commitment_data
                    .iter()
                    .map(|(a, b)| (a, b.as_ref().map(|b| b.as_ref()))),
                logger
            );

            let htlc_claim_reqs =
                self.get_counterparty_htlc_output_claim_reqs(commitment_number, commitment_txid, Some(tx));
        ...
        L::Target: Logger,
    {
        ...
        if self.current_holder_commitment_tx.txid == commitment_txid {
            ...
            log_info!(
                logger,
                "Got broadcast of latest holder commitment tx {}, searching for available HTLCs to claim",
                commitment_txid
            );
            ...
            if holder_tx.txid == commitment_txid {
                ...
                log_info!(
                    logger,
                    "Got broadcast of previous holder commitment tx {}, searching for available HTLCs to claim",
                    commitment_txid
                );
        ...
        L::Target: Logger,
    {
        ...
        let commitment_tx = self
            .onchain_tx_handler
            .get_fully_signed_holder_tx(&self.funding_redeemscript);
        ...
        L::Target: Logger,
    {
        ...
        let commitment_tx = self
            .onchain_tx_handler
            .get_fully_signed_copy_holder_tx(&self.funding_redeemscript);
        let txid = commitment_tx.txid();
        let mut holder_transactions = vec![commitment_tx];
        for htlc in self.current_holder_commitment_tx.htlc_outputs.iter() {
            if let Some(vout) = htlc.0.transaction_output_index {
        ...
        L::Target: Logger,
    {
        ...
        if height > self.best_block.height() {
            ...
            self.onchain_events_awaiting_threshold_conf
                .retain(|ref entry| entry.height <= height);
            self.onchain_tx_handler
                .block_disconnected(height + 1, broadcaster, fee_estimator, logger);
            Vec::new()
        } else {
            Vec::new()
        }
                ...
                if prevout.txid == self.funding_info.0.txid && prevout.vout == self.funding_info.0.index as u32 {
                    ...
                    if let Some(&commitment_number) = self.counterparty_commitment_txn_on_chain.get(&prevout.txid) {
                        let (mut new_outpoints, new_outputs_option) =
                            self.check_spend_counterparty_htlc(&tx, commitment_number, height, &logger);
                        claimable_outpoints.append(&mut new_outpoints);
        ...
        L::Target: Logger,
    {
        ...
        if should_broadcast {
            ...
            let (mut new_outpoints, _) =
                self.get_broadcasted_holder_claims(&self.current_holder_commitment_tx, self.best_block.height());
            let new_outputs =
                self.get_broadcasted_holder_watch_outputs(&self.current_holder_commitment_tx, &commitment_tx);
        ...
        L::Target: Logger,
    {
        ...
        self.onchain_tx_handler
            .block_disconnected(height, broadcaster, fee_estimator, logger);
    ...
    {
        self.onchain_events_awaiting_threshold_conf
            .retain(|ref entry| entry.txid != *txid);
        self.onchain_tx_handler
            .transaction_unconfirmed(txid, broadcaster, fee_estimator, logger);
    }
                            ...
                            if _script_pubkey.is_v0_p2wsh() {
                                ...
                                assert_eq!(
                                    &bitcoin::Address::p2wpkh(
                                        &bitcoin::PublicKey::from_slice(&input.witness.last().unwrap()).unwrap(),
                                        bitcoin::Network::Bitcoin
                                    )
                                    .unwrap()
                                    .script_pubkey(),
        ...
        L::Target: Logger,
    {
        'outer_loop: for input in &tx.input {
            ...
            let offered_timeout_claim = input.witness.len() == 5
                && HTLCType::scriptlen_to_htlctype(input.witness[4].len()) == Some(HTLCType::OfferedHTLC);
            ...
            macro_rules! log_claim {
                ($tx_info: expr, $holder_tx: expr, $htlc: expr, $source_avail: expr) => {
                    ...
                    debug_assert!(
                        !$htlc.offered || offered_preimage_claim || offered_timeout_claim || revocation_sig_claim
                    );
                    #[cfg(not(fuzzing))] // Note that the fuzzer is not bound by pesky things like "signatures"
                    debug_assert!(
                        $htlc.offered || accepted_preimage_claim || accepted_timeout_claim || revocation_sig_claim
                    );
                    // Further, only exactly one of the possible spend paths should have been
                    // matched by any HTLC spend:
                    #[cfg(not(fuzzing))] // Note that the fuzzer is not bound by pesky things like "signatures"
                    debug_assert_eq!(
                        accepted_preimage_claim as u8
                            + accepted_timeout_claim as u8
                            + offered_preimage_claim as u8
                            + offered_timeout_claim as u8
                            + revocation_sig_claim as u8,
                        1
                    );
                        ...
                        || (outbound_htlc && !$source_avail && (accepted_preimage_claim || offered_preimage_claim))
                    {
                        log_error!(
                            logger,
                            "Input spending {} ({}:{}) in {} resolves {} HTLC with payment hash {} with {}!",
                            $tx_info,
                            input.previous_output.txid,
                            input.previous_output.vout,
                            tx.txid(),
                            if outbound_htlc { "outbound" } else { "inbound" },
                            log_bytes!($htlc.payment_hash.0),
                            if revocation_sig_claim {
                                "revocation sig"
                            } else {
                                "preimage claim after we'd passed the HTLC resolution back"
                            }
                        );
                    } else {
                        log_info!(
                            ...
                            if revocation_sig_claim {
                                "revocation sig"
                            } else if accepted_preimage_claim || offered_preimage_claim {
                                "preimage"
                            } else {
                                "timeout"
                            }
        ...
        L::Target: Logger,
    {
        ...
        if let Some(spendable_output) = spendable_output {
            let entry = OnchainEventEntry {
                txid: tx.txid(),
                height,
                event: OnchainEvent::MaturingOutput {
                    descriptor: spendable_output.clone(),
                },
            };
            log_info!(
                logger,
    ...
    L::Target: Logger,
{
    fn block_connected(&self, block: &Block, height: u32) {
        ...
        self.0
            .block_connected(&block.header, &txdata, height, &*self.1, &*self.2, &*self.3);
    }
...
impl<'a, Signer: Sign, K: KeysInterface<Signer = Signer>> ReadableArgs<&'a K> for (BlockHash, ChannelMonitor<Signer>) {
    fn read<R: io::Read>(reader: &mut R, keys_manager: &'a K) -> Result<Self, DecodeError> {
        ...
        let mut prev_holder_signed_commitment_tx: Option<HolderSignedTx> = match <u8 as Readable>::read(reader)? {
            0 => None,
        ...
        let mut outputs_to_watch = HashMap::with_capacity(cmp::min(
            ...
            MAX_ALLOC_SIZE / (mem::size_of::<Txid>() + mem::size_of::<u32>() + mem::size_of::<Vec<Script>>()),
        ));
...
mod tests {
    ...
    use chain::package::{
        weight_offered_htlc, weight_received_htlc, weight_revoked_offered_htlc, weight_revoked_received_htlc,
    ...
    use {
        check_added_monitors, check_closed_broadcast, check_closed_event, check_spends, get_local_commitment_txn,
        get_monitor, get_route_and_payment_hash, unwrap_send_err,
    };
    ...
    fn do_test_funding_spend_refuses_updates(use_local_txn: bool) {
        ...
        nodes[1]
            .chain_monitor
            .chain_monitor
            .transactions_confirmed(&new_header, &[(0, broadcast_tx)], conf_height);
        ...
        let mut replay_update = monitor_updates
            .get(&channel.2)
            .unwrap()
            .iter()
            .rev()
    ...
    fn test_prune_preimages() {
        ...
        let fee_estimator = Arc::new(TestFeeEstimator {
        ...
        macro_rules! sign_input {
            ($sighash_parts: expr, $idx: expr, $amount: expr, $weight: expr, $sum_actual_sigs: expr, $opt_anchors: expr) => {
                ...
                let redeem_script = if *$weight == WEIGHT_REVOKED_OUTPUT {
                    ...
                    chan_utils::get_htlc_redeemscript_with_explicit_keys(&htlc, $opt_anchors, &pubkey, &pubkey, &pubkey)
                };
                    ...
                    || *$weight == weight_revoked_received_htlc($opt_anchors)
                {
                    $sighash_parts
                        .access_witness($idx)
                        .push(pubkey.clone().serialize().to_vec());
                } else if *$weight == weight_received_htlc($opt_anchors) {
                    $sighash_parts.access_witness($idx).push(vec![0]);
                } else {
                    $sighash_parts
                        .access_witness($idx)
                        .push(PaymentPreimage([1; 32]).0.to_vec());
                }
                $sighash_parts
                    .access_witness($idx)
                    .push(redeem_script.into_bytes());
