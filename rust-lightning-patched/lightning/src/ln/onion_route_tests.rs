...
use ln::channelmanager::{
    HTLCForwardInfo, PendingHTLCInfo, PendingHTLCRouting, CLTV_FAR_FAR_AWAY, MIN_CLTV_EXPIRY_DELTA,
};
    ...
    F3: FnMut(),
{
    ...
    nodes[0]
        .node
        .send_payment(&route, payment_hash.clone(), &Some(*payment_secret))
        .unwrap();
            ...
            match network_update {
                Some(update) => match update {
                    &NetworkUpdate::ChannelUpdateMessage { .. } => {
                        if let NetworkUpdate::ChannelUpdateMessage { .. } = expected_channel_update.unwrap() {
                        } else {
                    ...
                    &NetworkUpdate::ChannelClosed {
                        ...
                        if let NetworkUpdate::ChannelClosed {
...
fn test_fee_failures() {
    ...
    nodes[0]
        .node
        .send_payment(&route, payment_hash_success, &Some(payment_secret_success))
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    pass_along_route(
        &nodes[0],
...
fn test_onion_failure() {
    ...
    run_onion_failure_test(
        ...
        |msg| {
            ...
            let (mut onion_payloads, _htlc_msat, _htlc_cltv) =
                onion_utils::build_onion_payloads(&route.paths[0], 40000, &None, cur_height, &None).unwrap();
    ...
    run_onion_failure_test_with_fail_intercept(
        ...
        Some(NetworkUpdate::NodeFailure {
            node_id: route.paths[0][0].pubkey,
            is_permanent: false,
        }),
        Some(route.paths[0][0].short_channel_id),
    );
    ...
    run_onion_failure_test_with_fail_intercept(
        ...
        Some(NODE | 2),
        Some(NetworkUpdate::NodeFailure {
            node_id: route.paths[0][1].pubkey,
            is_permanent: false,
        }),
        Some(route.paths[0][1].short_channel_id),
    );
    let (_, payment_hash, payment_secret) = get_payment_preimage_hash!(nodes[2]);
    ...
    run_onion_failure_test_with_fail_intercept(
        ...
        |msg| {
            ...
            msg.reason = onion_utils::build_first_hop_failure_packet(
                ...
                UPDATE | 7,
                &ChannelUpdate::dummy(short_channel_id).encode_with_len()[..],
            );
        },
        || {},
        true,
        ...
        Some(NetworkUpdate::ChannelUpdateMessage {
            msg: ChannelUpdate::dummy(short_channel_id),
        }),
        Some(short_channel_id),
    );
    ...
    run_onion_failure_test(
        ...
        |msg| {
            ...
            let (onion_payloads, _, htlc_cltv) =
                onion_utils::build_onion_payloads(&route.paths[0], 40000, &None, height, &None).unwrap();
...
fn test_phantom_failure_too_low_recv_amt() {
    ...
    error_data.extend_from_slice(&byte_utils::be32_to_array(
        nodes[1].node.best_block.read().unwrap().height(),
    ));
    let mut fail_conditions = PaymentFailedConditions::new()
        .blamed_scid(phantom_scid)
