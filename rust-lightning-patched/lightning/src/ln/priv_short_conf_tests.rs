...
use ln::msgs;
use ln::msgs::{ChannelMessageHandler, OptionalField, RoutingMessageHandler};
...
fn test_priv_forwarding_rejection() {
    ...
    let chan_id_1 = create_announced_chan_between_nodes_with_value(
        &nodes,
        0,
        1,
        1_000_000,
    ...
    .2;
    let chan_id_2 = create_unannounced_chan_between_nodes_with_value(
        &nodes,
        1,
        2,
        1_000_000,
    ...
    let (route, our_payment_hash, our_payment_preimage, our_payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[2], last_hops, 10_000, TEST_FINAL_CLTV);
    ...
    get_monitor!(nodes[1], chan_id_1)
        .write(&mut monitor_a_serialized)
        .unwrap();
    get_monitor!(nodes[1], chan_id_2)
        .write(&mut monitor_b_serialized)
        .unwrap();
    ...
    let (_, mut monitor_a) =
        <(BlockHash, ChannelMonitor<EnforcingSigner>)>::read(&mut monitor_a_read, keys_manager).unwrap();
    let (_, mut monitor_b) =
        <(BlockHash, ChannelMonitor<EnforcingSigner>)>::read(&mut monitor_b_read, keys_manager).unwrap();
    ...
    assert!(nodes[1]
        .chain_monitor
        .watch_channel(monitor_a.get_funding_txo().0, monitor_a)
        .is_ok());
    assert!(nodes[1]
        .chain_monitor
        .watch_channel(monitor_b.get_funding_txo().0, monitor_b)
        .is_ok());
    ...
    let cs_reestablish = get_event_msg!(
        nodes[2],
        MessageSendEvent::SendChannelReestablish,
        nodes[1].node.get_our_node_id()
    );
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[2].node.get_our_node_id(), &cs_reestablish);
    get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelUpdate,
        nodes[2].node.get_our_node_id()
...
fn do_test_1_conf_open(connect_style: ConnectStyle) {
    ...
    *nodes[0].connect_style.borrow_mut() = connect_style;
    ...
    let bs_announcement_sigs =
        if let MessageSendEvent::SendAnnouncementSignatures { ref node_id, ref msg } = bs_announce_events[0] {
            assert_eq!(*node_id, nodes[0].node.get_our_node_id());
            msg.clone()
        } else {
            panic!("Unexpected event");
...
fn test_routed_scid_alias() {
    ...
    .2;
    let mut as_funding_locked = create_unannounced_chan_between_nodes_with_value(
        &nodes,
        1,
        2,
        1_000_000,
    ...
    let (route, payment_hash, payment_preimage, payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[2], hop_hints, 100_000, 42);
    assert_eq!(
        route.paths[0][1].short_channel_id,
...
fn test_inbound_scid_privacy() {
    ...
    nodes[1]
        .node
        .handle_accept_channel(&nodes[2].node.get_our_node_id(), InitFeatures::known(), &accept_channel);
    ...
    nodes[1]
        .node
        .handle_funding_signed(&nodes[2].node.get_our_node_id(), &cs_funding_signed);
    ...
    let bs_funding_locked = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendFundingLocked,
        nodes[2].node.get_our_node_id()
    );
    ...
    nodes[1]
        .node
        .handle_channel_update(&nodes[2].node.get_our_node_id(), &cs_update);
    nodes[2]
        .node
    ...
    let (route, payment_hash, payment_preimage, payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[2], hop_hints.clone(), 100_000, 42);
    assert_eq!(
        route.paths[0][1].short_channel_id,
        last_hop[0].inbound_scid_alias.unwrap()
    );
    ...
    let (route_2, payment_hash_2, _, payment_secret_2) =
        get_route_and_payment_hash!(nodes[0], nodes[2], hop_hints, 100_000, 42);
    assert_eq!(
        route_2.paths[0][1].short_channel_id,
    ...
    nodes[1].logger.assert_log_regex(
        ...
        regex::Regex::new(r"Refusing to forward over real channel SCID as our counterparty requested").unwrap(),
        1,
    );
...
fn test_scid_alias_returned() {
    ...
    let (mut route, payment_hash, _, payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[2], hop_hints, 10_000, 42);
    assert_eq!(
        route.paths[0][1].short_channel_id,
    ...
    let signature = Secp256k1::new().sign(
        &hash_to_message!(&msg_hash[..]),
        &nodes[1].keys_manager.get_node_secret(Recipient::Node).unwrap(),
    );
    ...
    expect_payment_failed_conditions!(
        ...
        PaymentFailedConditions::new()
            ...
            .expected_htlc_error_data(0x1000 | 7, &msg.encode_with_len())
    );
