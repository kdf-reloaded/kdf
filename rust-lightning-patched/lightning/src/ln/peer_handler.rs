...
pub struct MessageHandler<CM: Deref, RM: Deref>
where
...
///
/// (C-not exported) as Arcs don't make sense in bindings
pub type SimpleRefPeerManager<'a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, SD, M, T, F, C, L> = PeerManager<
    SD,
    SimpleRefChannelManager<'a, 'b, 'c, 'd, 'e, M, T, F, L>,
    &'e NetGraphMsgHandler<&'g NetworkGraph, &'h C, &'f L>,
    &'f L,
    IgnoringMessageHandler,
>;
...
pub struct PeerManager<Descriptor: SocketDescriptor, CM: Deref, RM: Deref, L: Deref, CMH: Deref>
where
    CM::Target: ChannelMessageHandler,
    RM::Target: RoutingMessageHandler,
    L::Target: Logger,
...
impl<Descriptor: SocketDescriptor, CM: Deref, L: Deref>
    PeerManager<Descriptor, CM, IgnoringMessageHandler, L, IgnoringMessageHandler>
where
    CM::Target: ChannelMessageHandler,
    L::Target: Logger,
{
...
impl<Descriptor: SocketDescriptor, RM: Deref, L: Deref>
    PeerManager<Descriptor, ErroringMessageHandler, RM, L, IgnoringMessageHandler>
where
    RM::Target: RoutingMessageHandler,
    L::Target: Logger,
{
...
impl<Descriptor: SocketDescriptor, CM: Deref, RM: Deref, L: Deref, CMH: Deref> PeerManager<Descriptor, CM, RM, L, CMH>
where
    CM::Target: ChannelMessageHandler,
    RM::Target: RoutingMessageHandler,
    L::Target: Logger,
    CMH::Target: CustomMessageHandler,
{
                ...
                && peer.msgs_sent_since_pong < BUFFER_DRAIN_MSGS_PER_TICK
            {
                match peer.sync_status {
                    ...
                    InitSyncTracker::ChannelsSyncing(c) if c < 0xffff_ffff_ffff_ffff => {
                        let steps =
                            ((OUTBOUND_BUFFER_LIMIT_READ_PAUSE - peer.pending_outbound_buffer.len() + 2) / 3) as u8;
                        let all_messages = self
                            .message_handler
                            .route_handler
                            .get_next_channel_announcements(c, steps);
    ...
    pub fn read_event(&self, peer_descriptor: &mut Descriptor, data: &[u8]) -> Result<bool, PeerHandleError> {
        match self.do_read_event(peer_descriptor, data) {
            ...
            Err(e) => {
                log_trace!(
                    self.logger,
                    "Peer sent invalid data or we decided to disconnect due to a protocol error"
                );
    ...
    fn enqueue_encoded_message(&self, peer: &mut Peer, encoded_message: &Vec<u8>) {
        ...
        peer.pending_outbound_buffer
            .push_back(peer.channel_encryptor.encrypt_message(&encoded_message[..]));
    }
        ...
        let pause_read = {
            ...
            let pause_read = match peers.peers.get_mut(peer_descriptor) {
                ...
                Some(peer) => {
                                    ...
                                    if peer.pending_read_is_header {
                                        ...
                                        let message = match message_result {
                                            ...
                                            Err(e) => {
                                                match e {
                                                    ...
                                                    (msgs::DecodeError::UnknownRequiredFeature, Some(ty))
                                                        if is_gossip_msg(ty) =>
                                                    {
                                                    ...
                                                    (msgs::DecodeError::UnsupportedCompression, _) => {
                                                        ...
                                                        self.enqueue_message(
                                                            ...
                                                            &msgs::WarningMessage {
                                                                channel_id: [0; 32],
                                                                data: "Unsupported message compression: zlib"
                                                                    .to_owned(),
                                                            },
                                                        );
                                                        continue;
                                                    ...
                                                    (_, Some(ty)) if is_gossip_msg(ty) => {
                                                        ...
                                                        self.enqueue_message(
                                                            ...
                                                            &msgs::WarningMessage {
                                                                channel_id: [0; 32],
                                                                data: "Unreadable/bogus gossip message".to_owned(),
                                                            },
                                                        );
                                                        continue;
                                                    },
                                        ...
                                        match self.handle_message(peer, message) {
                                            ...
                                            Ok(Some(msg)) => {
                                                peer_node_id = Some(
                                                    peer.their_node_id
                                                        .expect("After noise is complete, their_node_id is always set"),
                                                );
                    ...
                    peer.pending_outbound_buffer.len() > OUTBOUND_BUFFER_LIMIT_READ_PAUSE
                    // pause_read
                },
            };
    ...
    fn handle_message(
        ...
        Option<wire::Message<<<CMH as core::ops::Deref>::Target as wire::CustomMessageReader>::CustomMessage>>,
        ...
        match message {
            ...
            wire::Message::Init(msg) => {
                ...
                if !msg.features.supports_static_remote_key() {
                    log_debug!(
                        self.logger,
                        "Peer {} does not support static remote key, disconnecting with no_connection_possible",
                        log_pubkey!(peer.their_node_id.unwrap())
                    );
                    return Err(PeerHandleError {
                        no_connection_possible: true,
            ...
            wire::Message::Warning(msg) => {
                ...
                if data_is_printable {
                    ...
                    log_debug!(
                        self.logger,
                        "Got warning message from {} with non-ASCII error message",
                        log_pubkey!(peer.their_node_id.unwrap())
                    );
                }
            },
            ...
            wire::Message::Shutdown(msg) => {
                self.message_handler.chan_handler.handle_shutdown(
                    &peer.their_node_id.unwrap(),
                    peer.their_features.as_ref().unwrap(),
                    &msg,
                );
            },
            ...
            wire::Message::UpdateFailMalformedHTLC(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_update_fail_malformed_htlc(&peer.their_node_id.unwrap(), &msg);
            },
            ...
            wire::Message::ChannelUpdate(msg) => {
                self.message_handler
                    .chan_handler
                    .handle_channel_update(&peer.their_node_id.unwrap(), &msg);
                if self
                    .message_handler
                    .route_handler
            ...
            wire::Message::QueryShortChannelIds(msg) => {
                self.message_handler
                    .route_handler
                    .handle_query_short_channel_ids(&peer.their_node_id.unwrap(), msg)?;
            },
            wire::Message::ReplyShortChannelIdsEnd(msg) => {
                self.message_handler
                    .route_handler
                    .handle_reply_short_channel_ids_end(&peer.their_node_id.unwrap(), msg)?;
            },
            wire::Message::QueryChannelRange(msg) => {
                self.message_handler
                    .route_handler
                    .handle_query_channel_range(&peer.their_node_id.unwrap(), msg)?;
            },
            wire::Message::ReplyChannelRange(msg) => {
                self.message_handler
                    .route_handler
                    .handle_reply_channel_range(&peer.their_node_id.unwrap(), msg)?;
            },
            ...
            wire::Message::Custom(custom) => {
                self.custom_message_handler
                    .handle_custom_message(custom, &peer.their_node_id.unwrap())?;
            },
        };
        Ok(should_forward)
    }
    ...
    fn forward_broadcast_msg(
        &self,
        peers: &mut PeerHolder<Descriptor>,
        msg: &wire::Message<<<CMH as core::ops::Deref>::Target as wire::CustomMessageReader>::CustomMessage>,
            ...
            for event in events_generated.drain(..) {
                match event {
                    ...
                    MessageSendEvent::SendChannelUpdate { ref node_id, ref msg } => {
                        log_trace!(
                            self.logger,
                            "Handling SendChannelUpdate event in peer_handler for node {} for channel {}",
                            log_pubkey!(node_id),
                            msg.contents.short_channel_id
                        );
                        let peer = get_peer_for_forwarding!(node_id);
            ...
            Some(peer) => match peer.their_node_id {
                Some(node_id) => {
                    log_trace!(
                        ...
                        log_pubkey!(node_id),
                        if no_connection_possible { "no " } else { "" }
                    );
    ...
    pub fn disconnect_by_node_id(&self, node_id: PublicKey, no_connection_possible: bool) {
        ...
        if let Some(mut descriptor) = peers_lock.node_id_to_descriptor.remove(&node_id) {
            log_trace!(
                self.logger,
                "Disconnecting peer with id {} due to client request",
                node_id
            );
    ...
    pub fn disconnect_all_peers(&self) {
        ...
        for (mut descriptor, peer) in peers.peers.drain() {
            if let Some(node_id) = peer.their_node_id {
                log_trace!(
                    self.logger,
                    "Disconnecting peer with id {} due to client request to disconnect all peers",
                    node_id
                );
                peers.node_id_to_descriptor.remove(&node_id);
                self.message_handler.chan_handler.peer_disconnected(&node_id, false);
                        ...
                        > MAX_BUFFER_DRAIN_TICK_INTERVALS_PER_PEER as u64 * peer_count as u64
                {
                    ...
                    match peer.their_node_id {
                        Some(node_id) => {
                            log_trace!(
                                self.logger,
                                "Disconnecting peer with id {} due to ping timeout",
                                node_id
                            );
...
fn is_gossip_msg(type_id: u16) -> bool {
    match type_id {
        msgs::ChannelAnnouncement::TYPE | msgs::ChannelUpdate::TYPE | msgs::NodeAnnouncement::TYPE => true,
        _ => false,
    }
}
...
mod tests {
    ...
    use ln::peer_handler::{filter_addresses, IgnoringMessageHandler, MessageHandler, PeerManager, SocketDescriptor};
    use util::events;
    use util::test_utils;
    ...
    fn establish_connection<'a>(
        ...
        assert_eq!(
            peer_b
                .read_event(&mut fd_b, &fd_a.outbound_data.lock().unwrap().split_off(0))
                .unwrap(),
            false
        );
        ...
        assert_eq!(
            peer_a
                .read_event(&mut fd_a, &fd_b.outbound_data.lock().unwrap().split_off(0))
                .unwrap(),
            false
        );
    ...
    fn test_do_attempt_write_data() {
        ...
        for _ in 0..150 / super::BUFFER_DRAIN_MSGS_PER_TICK + 1 {
            ...
            peers[1].process_events();
            assert_eq!(
                fd_b.outbound_data.lock().unwrap().len(),
                0,
                "Until B receives data, it shouldn't send more messages"
            );
        }
    ...
    fn test_filter_addresses() {
        ...
        // For (10/8)
        let ip_address = NetAddress::IPv4 {
            addr: [10, 0, 0, 0],
            port: 1000,
        };
        ...
        // For (2000::/3)
        let ip_address = NetAddress::IPv6 {
