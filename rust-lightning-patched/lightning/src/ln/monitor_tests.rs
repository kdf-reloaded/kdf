...
use chain::channelmonitor::{Balance, ANTI_REORG_DELAY};
...
fn chanmon_fail_from_stale_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &fail_updates.update_fail_htlcs[0]);
...
fn chanmon_claim_value_coop_close() {
    ...
    assert_eq!(
        vec![Balance::ClaimableOnChannelClose {
            claimable_amount_satoshis: 1_000_000
                - 1_000
                - chan_feerate * channel::commitment_tx_base_weight(opt_anchors) / 1000
        }],
        nodes[0]
            .chain_monitor
            .chain_monitor
...
fn do_test_claim_value_force_close(prev_commitment_tx: bool) {
        ...
        sorted_vec(vec![
            Balance::ClaimableOnChannelClose {
                claimable_amount_satoshis: 1_000_000
                    ...
                    - 3
                    - chan_feerate
                        * (channel::commitment_tx_base_weight(opt_anchors)
    ...
    if prev_commitment_tx {
        ...
        nodes[0]
            .node
            .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &b_htlc_msgs.update_fulfill_htlcs[0]);
        expect_payment_sent_without_paths!(nodes[0], payment_preimage);
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &b_htlc_msgs.commitment_signed);
        check_added_monitors!(nodes[0], 1);
        ...
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_cs);
    ...
    connect_blocks(&nodes[0], ANTI_REORG_DELAY - 1);
    assert_eq!(
        vec![Balance::MaybeClaimableHTLCAwaitingTimeout {
            claimable_amount_satoshis: 4_000,
            claimable_height: htlc_cltv_timeout,
        }],
        nodes[0]
    ...
    assert_eq!(
        remote_txn[0].output[a_broadcast_txn[2].input[0].previous_output.vout as usize].value,
        4_000
    );
    ...
    assert_eq!(
        vec![Balance::ContentiousClaimable {
            claimable_amount_satoshis: 4_000,
            timeout_height: htlc_cltv_timeout,
        }],
        nodes[1]
