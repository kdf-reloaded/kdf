...
use chain::channelmonitor::{ChannelMonitor, ANTI_REORG_DELAY, LATENCY_GRACE_PERIOD_BLOCKS};
use chain::keysinterface::KeysInterface;
use chain::transaction::OutPoint;
use chain::{ChannelMonitorUpdateErr, Confirm, Listen, Watch};
use io;
use ln::channelmanager::{
    ChannelManager, ChannelManagerReadArgs, PaymentId, PaymentSendFailure, BREAKDOWN_TIMEOUT, MPP_TIMEOUT_TICKS,
};
...
fn mpp_retry() {
    ...
    let chan_3_id = create_announced_chan_between_nodes(&nodes, 1, 3, InitFeatures::known(), InitFeatures::known())
        ...
        .contents
        .short_channel_id;
    let chan_4_id = create_announced_chan_between_nodes(&nodes, 3, 2, InitFeatures::known(), InitFeatures::known())
        .0
        .contents
        .short_channel_id;
    // Rebalance
    ...
    let (mut route, payment_hash, payment_preimage, payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[3], 1_000_000);
    let path = route.paths[0].clone();
    route.paths.push(path);
    route.paths[0][0].pubkey = nodes[1].node.get_our_node_id();
    route.paths[0][0].short_channel_id = chan_1_id;
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[2].node.get_our_node_id(), &htlc_updates.update_fail_htlcs[0]);
    ...
    if let Err(PaymentSendFailure::ParameterError(APIError::APIMisuseError { err })) =
        nodes[0].node.retry_payment(&route, payment_id)
    {
    ...
    if let Err(PaymentSendFailure::ParameterError(APIError::APIMisuseError { err })) =
        nodes[0].node.retry_payment(&route, PaymentId([0; 32]))
    {
...
fn do_retry_with_no_persist(confirm_before_reload: bool) {
    ...
    let (payment_preimage_1, _, _, payment_id_1) =
    ...
    nodes[1]
        ...
        .unwrap()
        .config
        .forwarding_fee_base_msat += 100_000;
...
fn do_test_dup_htlc_onchain_fails_on_reload(
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .channel_monitor_updated(funding_txo, mon_updates[0])
        .unwrap();
    if payment_timeout {
    ...
    let (_, nodes_0_deserialized_tmp) = {
        ...
        <(
            ...
            &mut io::Cursor::new(&chan_manager_serialized.0[..]),
            ChannelManagerReadArgs {
...
fn get_ldk_payment_preimage() {
    ...
    let (payment_hash, payment_secret) = nodes[1]
        .node
        .create_inbound_payment(Some(amt_msat), expiry_secs)
        .unwrap();
