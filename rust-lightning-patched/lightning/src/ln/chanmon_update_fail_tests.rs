...
use chain::channelmonitor::{ChannelMonitor, ANTI_REORG_DELAY};
use chain::transaction::OutPoint;
use chain::{ChannelMonitorUpdateErr, Listen, Watch};
use ln::channel::AnnouncementSigsState;
use ln::channelmanager::{ChannelManager, ChannelManagerReadArgs, PaymentSendFailure, RAACommitmentOrder};
...
fn test_simple_monitor_permanent_update_fail() {
    ...
    chanmon_cfgs[0]
        .persister
        .set_update_ret(Err(ChannelMonitorUpdateErr::PermanentFailure));
    unwrap_send_err!(
        nodes[0]
            .node
    ...
    check_closed_event!(
        ...
        ClosureReason::ProcessingError {
            err: "ChannelMonitor storage failure".to_string()
        }
    );
}
...
fn test_monitor_and_persister_update_fail() {
    ...
    if let Some(ref mut channel) = nodes[0].node.channel_state.lock().unwrap().by_id.get_mut(&chan.2) {
        if let Ok((_, _, update)) = channel.commitment_signed(&updates.commitment_signed, &node_cfgs[0].logger) {
            ...
            if let Err(ChannelMonitorUpdateErr::PermanentFailure) =
                chain_mon.chain_monitor.update_channel(outpoint, update.clone())
            {
            } else {
                panic!("Expected monitor error to be permanent");
            }
            logger.assert_log_regex(
                "lightning::chain::chainmonitor".to_string(),
                regex::Regex::new("Failed to persist ChannelMonitor update for channel [0-9a-f]*: TemporaryFailure")
                    .unwrap(),
                1,
            );
...
fn do_test_simple_monitor_temporary_update_fail(disconnect: bool) {
    ...
    chanmon_cfgs[0]
        .persister
        .set_update_ret(Err(ChannelMonitorUpdateErr::TemporaryFailure));
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(outpoint, latest_update);
...
fn test_monitor_update_fail_cs() {
    ...
    match responses[0] {
        MessageSendEvent::SendRevokeAndACK { ref msg, ref node_id } => {
            ...
            nodes[0]
                .node
                .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &msg);
            check_added_monitors!(nodes[0], 1);
        },
        _ => panic!("Unexpected event"),
    }
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &final_raa);
    check_added_monitors!(nodes[1], 1);
...
fn test_monitor_update_fail_no_rebroadcast() {
        ...
        get_route_and_payment_hash!(nodes[0], nodes[1], 1000000);
    {
        nodes[0]
            .node
            .send_payment(&route, our_payment_hash, &Some(payment_secret_1))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    }
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &bs_raa);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    nodes[1].logger.assert_log(
        "lightning::ln::channelmanager".to_string(),
        "Failed to update ChannelMonitor".to_string(),
...
fn test_monitor_update_raa_while_paused() {
        ...
        get_route_and_payment_hash!(nodes[0], nodes[1], 1000000);
    {
        nodes[0]
            .node
            .send_payment(&route, our_payment_hash_1, &Some(our_payment_secret_1))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    }
        ...
        get_route_and_payment_hash!(nodes[1], nodes[0], 1000000);
    {
        nodes[1]
            .node
            .send_payment(&route, our_payment_hash_2, &Some(our_payment_secret_2))
            .unwrap();
        check_added_monitors!(nodes[1], 1);
    }
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(&nodes[0].node.get_our_node_id(), &send_event_1.msgs[0]);
    nodes[1]
        .node
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &send_event_2.msgs[0]);
    nodes[0]
        .node
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_update_raa.0);
    check_added_monitors!(nodes[1], 1);
    let bs_cs = get_htlc_update_msgs!(nodes[1], nodes[0].node.get_our_node_id());

    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_update_raa.1);
    check_added_monitors!(nodes[1], 1);
...
fn do_test_monitor_update_fail_raa(test_ignore_second_cs: bool) {
    ...
    let (payment_preimage_4, payment_hash_4) = if test_ignore_second_cs {
        ...
        nodes[1]
            .node
            .handle_update_add_htlc(&nodes[2].node.get_our_node_id(), &send_event.msgs[0]);
        nodes[1]
            .node
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &messages_a.0);
    commitment_signed_dance!(nodes[0], nodes[1], messages_a.1, false);
    expect_payment_failed!(nodes[0], payment_hash_1, true);

    nodes[2]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &send_event_b.msgs[0]);
    let as_cs;
    if test_ignore_second_cs {
        nodes[2]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &send_event_b.commitment_msg);
        check_added_monitors!(nodes[2], 1);
        let bs_revoke_and_ack = get_event_msg!(
            nodes[2],
            MessageSendEvent::SendRevokeAndACK,
        ...
        nodes[2]
            .node
            .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &raa.unwrap());
        check_added_monitors!(nodes[2], 1);
        ...
        match bs_revoke_and_commit[0] {
            MessageSendEvent::SendRevokeAndACK { ref node_id, ref msg } => {
                ...
                nodes[1]
                    .node
                    .handle_revoke_and_ack(&nodes[2].node.get_our_node_id(), &msg);
                check_added_monitors!(nodes[1], 1);
            },
            _ => panic!("Unexpected event"),
        }
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[2].node.get_our_node_id(), &bs_second_raa);
    check_added_monitors!(nodes[1], 1);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());

    nodes[1]
        .node
        .handle_commitment_signed(&nodes[2].node.get_our_node_id(), &bs_second_cs.commitment_signed);
    check_added_monitors!(nodes[1], 1);
    let as_second_raa = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendRevokeAndACK,
    ...
    nodes[2]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &as_second_raa);
    check_added_monitors!(nodes[2], 1);
    ...
    if test_ignore_second_cs {
        ...
        nodes[0]
            .node
            .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &send_event.msgs[0]);
...
fn test_monitor_update_fail_reestablish() {
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(&nodes[2].node.get_our_node_id(), &updates.update_fulfill_htlcs[0]);
...
fn raa_no_response_awaiting_raa_state() {
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_responses.0);
    check_added_monitors!(nodes[0], 1);
    let mut events = nodes[0].node.get_and_clear_pending_msg_events();
    assert_eq!(events.len(), 1);
    let payment_event = SendEvent::from_event(events.pop().unwrap());
    ...
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_responses.1);
    check_added_monitors!(nodes[0], 1);
    let as_raa = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendRevokeAndACK,
...
fn claim_while_disconnected_monitor_update_fail() {
    ...
    let as_reconnect = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendChannelReestablish,
        nodes[1].node.get_our_node_id()
    );
    let bs_reconnect = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelReestablish,
        nodes[0].node.get_our_node_id()
    );
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &bs_reconnect);
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &as_reconnect);
    ...
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_commitment.commitment_signed);
    check_added_monitors!(nodes[0], 1);
    let as_raa = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendRevokeAndACK,
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_commitment.commitment_signed);
    check_added_monitors!(nodes[1], 1);
    let bs_raa = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendRevokeAndACK,
...
fn test_monitor_update_fail_claim() {
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &bs_forward_update.update_add_htlcs[0]);
    nodes[0]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &bs_forward_update.update_add_htlcs[1]);
...
fn test_monitor_update_on_pending_forwards() {
    ...
    nodes[1]
        .node
        .handle_update_fail_htlc(&nodes[2].node.get_our_node_id(), &cs_fail_update.update_fail_htlcs[0]);
...
fn do_during_funding_monitor_fail(confirm_a_first: bool, restore_b_before_conf: bool, restore_b_before_lock: bool) {
    ...
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), 100000, 10001, 43, None)
        .unwrap();
    ...
    let (channel_id, (announcement, as_update, bs_update)) = if !confirm_a_first {
        if !restore_b_before_lock {
            let (funding_locked, channel_id) =
                create_chan_between_nodes_with_value_confirm_second(&nodes[0], &nodes[1]);
            (
                channel_id,
                create_chan_between_nodes_with_value_b(&nodes[1], &nodes[0], &funding_locked),
            )
        } else {
...
fn test_path_paused_mpp() {
    ...
    chanmon_cfgs[0]
        .persister
        .set_next_update_ret(Some(Err(ChannelMonitorUpdateErr::TemporaryFailure)));
...
fn test_pending_update_fee_ack_on_reconnect() {
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_update_fee_msgs.commitment_signed);
    check_added_monitors!(nodes[1], 1);
    ...
    let as_connect_msg = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendChannelReestablish,
        nodes[1].node.get_our_node_id()
    );
    ...
    let bs_connect_msg = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelReestablish,
        nodes[0].node.get_our_node_id()
    );
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &as_connect_msg);
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &bs_connect_msg);
    get_event_msg!(
        nodes[0],
        MessageSendEvent::SendChannelUpdate,
        nodes[1].node.get_our_node_id()
    ...
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_second_cs);
    check_added_monitors!(nodes[0], 1);
    nodes[0]
        .node
...
fn test_fail_htlc_on_broadcast_after_claim() {
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(&nodes[2].node.get_our_node_id(), &cs_updates.update_fulfill_htlcs[0]);
    let bs_updates = get_htlc_update_msgs!(nodes[1], nodes[0].node.get_our_node_id());
    check_added_monitors!(nodes[1], 1);
    expect_payment_forwarded!(nodes[1], Some(1000), false);
...
fn do_update_fee_resend_test(deliver_update: bool, parallel_updates: bool) {
    ...
    if parallel_updates {
        nodes[1]
            .node
            .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &update_msgs.commitment_signed);
        check_added_monitors!(nodes[1], 1);
        ...
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_second_cs.commitment_signed);
        check_added_monitors!(nodes[0], 1);
        let as_second_raa = get_event_msg!(
            nodes[0],
            MessageSendEvent::SendRevokeAndACK,
...
fn do_channel_holding_cell_serialize(disconnect: bool, reload_a: bool) {
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(&nodes[0].node.get_our_node_id(), &send.msgs[0]);
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &send.commitment_msg);
    check_added_monitors!(nodes[1], 1);
    ...
    if disconnect {
        ...
        if reload_a {
            ...
            nodes[0]
                .chain_monitor
                .watch_channel(chan_0_monitor.get_funding_txo().0.clone(), chan_0_monitor)
                .unwrap();
            check_added_monitors!(nodes[0], 1);
        } else {
            nodes[0].node.peer_disconnected(&nodes[1].node.get_our_node_id(), false);
    ...
    nodes[0]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(funding_txo, mon_id);
    ...
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &cs);
    check_added_monitors!(nodes[0], 1);
    ...
    let commitment_msg = match events.pop().unwrap() {
        MessageSendEvent::UpdateHTLCs { node_id, updates } => {
            ...
            nodes[1]
                .node
                .handle_update_fulfill_htlc(&nodes[0].node.get_our_node_id(), &updates.update_fulfill_htlcs[0]);
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &commitment_msg);
    check_added_monitors!(nodes[1], 1);
...
fn do_test_reconnect_dup_htlc_claims(htlc_status: HTLCStatusAtDupClaim, second_fails: bool) {
    ...
    if htlc_status == HTLCStatusAtDupClaim::HoldingCell {
        ...
        nodes[0]
            .node
            .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_cs);
        check_added_monitors!(nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(&nodes[2].node.get_our_node_id(), &fulfill_msg);
    expect_payment_forwarded!(nodes[1], Some(1000), false);
    check_added_monitors!(nodes[1], 1);
    ...
    if htlc_status == HTLCStatusAtDupClaim::HoldingCell {
        nodes[1]
            .node
            .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_raa.unwrap());
        check_added_monitors!(nodes[1], 1);
...
fn test_permanent_error_during_handling_shutdown() {
    ...
    nodes[1]
        .node
        .handle_shutdown(&nodes[0].node.get_our_node_id(), &InitFeatures::known(), &shutdown);
    check_closed_broadcast!(nodes[1], true);
...
fn double_temp_error() {
    ...
    nodes[1]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(funding_tx, latest_update_1);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 0);
    nodes[1]
        .chain_monitor
        .chain_monitor
        .force_channel_monitor_updated(funding_tx, latest_update_2);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &update_fulfill_1);
    check_added_monitors!(nodes[0], 0);
    expect_payment_sent_without_paths!(nodes[0], payment_preimage_1);
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &commitment_signed_b1);
    check_added_monitors!(nodes[0], 1);
    nodes[0].node.process_pending_htlc_forwards();
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &raa_a1);
    check_added_monitors!(nodes[1], 1);
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &commitment_signed_a1);
    check_added_monitors!(nodes[1], 1);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &raa_b2);
    check_added_monitors!(nodes[0], 1);
    expect_payment_path_successful!(nodes[0]);

    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &update_fulfill_2);
    check_added_monitors!(nodes[0], 0);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
