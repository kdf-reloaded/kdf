...
mod sealed {
    ...
    define_feature!(
        9,
        VariableLengthOnion,
        [InitContext, NodeContext, InvoiceContext],
    ...
    define_feature!(
        123456789,
        UnknownFeature,
        [NodeContext, ChannelContext, InvoiceContext],
        ...
        for (byte_idx, byte) in self.flags.iter().enumerate() {
            ...
            if new_u5_idx > 0 {
                ...
                res_u5s[new_u5_idx - 1] =
                    u5::try_from_u8(curr_u5_as_u8 | (((shifted_chunk_u16 >> 5) & 0x001f) as u8)).unwrap();
            }
            if new_u5_idx > 1 {
                let curr_u5_as_u8 = res_u5s[new_u5_idx - 2].to_u8();
                res_u5s[new_u5_idx - 2] =
                    u5::try_from_u8(curr_u5_as_u8 | (((shifted_chunk_u16 >> 10) & 0x001f) as u8)).unwrap();
            }
        }
