...
fn pre_funding_lock_shutdown_test() {
    ...
    nodes[1]
        .node
        .handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_2nd_closing_signed.unwrap());
...
fn updates_shutdown_wait() {
    ...
    unwrap_send_err!(
        nodes[0]
            .node
            .send_payment(&route_1, payment_hash, &Some(payment_secret)),
        true,
    ...
    unwrap_send_err!(
        nodes[1]
            .node
            .send_payment(&route_2, payment_hash, &Some(payment_secret)),
        true,
        APIError::ChannelUnavailable { .. },
        {}
    );
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &updates_2.update_fulfill_htlcs[0]);
...
fn do_test_shutdown_rebroadcast(recv_count: u8) {
    ...
    let node_0_reestablish = get_event_msg!(
        nodes[0],
    ...
    let node_1_reestablish = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelReestablish,
        nodes[0].node.get_our_node_id()
    );
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &node_0_reestablish);
    let node_1_2nd_shutdown = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendShutdown,
        nodes[0].node.get_our_node_id()
    );
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &node_1_reestablish);
    ...
    let node_1_2nd_reestablish = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelReestablish,
        nodes[0].node.get_our_node_id()
    );
    ...
    if recv_count == 0 {
        ...
        let node_0_2nd_reestablish = get_event_msg!(
            nodes[0],
            MessageSendEvent::SendChannelReestablish,
            nodes[1].node.get_our_node_id()
        );
        ...
        check_closed_event!(
            ...
            ClosureReason::CounterpartyForceClosed {
                peer_msg: "Failed to find corresponding channel".to_string()
            }
        );
    }
...
fn test_upfront_shutdown_script() {
    ...
    nodes[0]
        ...
        .unwrap();
    let node_0_orig_shutdown = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendShutdown,
    ...
    nodes[2].node.handle_shutdown(
        &nodes[0].node.get_our_node_id(),
        &InitFeatures::known(),
        &node_0_shutdown,
    );
    assert!(regex::Regex::new(
        r"Got shutdown request with a scriptpubkey \([A-Fa-f0-9]+\) which did not match their previous scriptpubkey."
    )
    .unwrap()
    .is_match(&check_warn_msg!(nodes[2], nodes[0].node.get_our_node_id(), chan.2)));
    // This allows nodes[2] to retry the shutdown message, which should get a response:
    nodes[2].node.handle_shutdown(
        &nodes[0].node.get_our_node_id(),
        &InitFeatures::known(),
        &node_0_orig_shutdown,
    );
    get_event_msg!(
        nodes[2],
        MessageSendEvent::SendShutdown,
...
fn test_unsupported_anysegwit_upfront_shutdown_script() {
    ...
    nodes[1]
        .node
        .handle_open_channel(&nodes[0].node.get_our_node_id(), node_features.clone(), &open_channel);
    ...
    nodes[0]
        .node
        .handle_accept_channel(&nodes[1].node.get_our_node_id(), node_features, &accept_channel);
...
fn test_invalid_upfront_shutdown_script() {
    ...
    open_channel.shutdown_scriptpubkey = Present(Builder::new().push_int(0).push_slice(&[0, 0]).into_script());
    nodes[0]
        .node
        .handle_open_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &open_channel);
...
fn test_unsupported_anysegwit_shutdown_script() {
    ...
    let chan = create_announced_chan_between_nodes(
        ...
        0,
        1,
        node_cfgs[0].features.clone(),
        node_cfgs[1].features.clone(),
    );
    match nodes[1].node.close_channel(
        &OutPoint {
            txid: chan.3.txid(),
            index: 0,
        }
...
fn do_test_closing_signed_reinit_timeout(timeout_step: TimeoutStep) {
    ...
    if timeout_step != TimeoutStep::AfterShutdown {
        ...
        {
            let mut lock;
            get_channel_ref!(nodes[0], lock, chan_id)
                .closing_fee_limits
                .as_mut()
                .unwrap()
                .1 *= 10;
        ...
        if timeout_step == TimeoutStep::NoTimeout {
            nodes[1]
                .node
                .handle_closing_signed(&nodes[0].node.get_our_node_id(), &node_0_2nd_closing_signed.1.unwrap());
            check_closed_event!(nodes[1], 1, ClosureReason::CooperativeClosure);
        }
        check_closed_event!(nodes[0], 1, ClosureReason::CooperativeClosure);
    }
    ...
    if timeout_step != TimeoutStep::NoTimeout {
        assert!(
            (txn[0].output[0].script_pubkey.is_v0_p2wpkh() && txn[0].output[1].script_pubkey.is_v0_p2wsh())
                || (txn[0].output[1].script_pubkey.is_v0_p2wpkh() && txn[0].output[0].script_pubkey.is_v0_p2wsh())
        );
...
fn simple_target_feerate_shutdown() {
    ...
    nodes[0]
        .node
        .close_channel_with_target_feerate(&chan_id, 253 * 10)
        .unwrap();
    let node_0_shutdown = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendShutdown,
    ...
    nodes[1]
        .node
        .close_channel_with_target_feerate(&chan_id, 253 * 5)
        .unwrap();
    let node_1_shutdown = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendShutdown,
