...
use chain::channelmonitor::{ChannelMonitor, ANTI_REORG_DELAY, CLTV_CLAIM_BUFFER, LATENCY_GRACE_PERIOD_BLOCKS};
use chain::keysinterface::{BaseSign, KeysInterface};
use chain::transaction::OutPoint;
use chain::{Confirm, Listen, Watch};
use ln::chan_utils::{htlc_success_tx_weight, htlc_timeout_tx_weight, HTLCOutputInCommitment};
use ln::channel::{
    commitment_tx_base_weight, COMMITMENT_TX_WEIGHT_PER_HTLC, CONCURRENT_INBOUND_HTLC_FEE_BUFFER,
    FEE_SPIKE_BUFFER_FEE_INCREASE_MULTIPLE, MIN_AFFORDABLE_HTLC_COUNT,
};
use ln::channel::{Channel, ChannelError};
use ln::channelmanager::{
    ChannelManager, ChannelManagerReadArgs, PaymentId, PaymentSendFailure, RAACommitmentOrder, BREAKDOWN_TIMEOUT,
    MIN_CLTV_EXPIRY_DELTA, PAYMENT_EXPIRY_BLOCKS,
};
...
use routing::router::{find_route, get_route, PaymentParameters, Route, RouteHop, RouteParameters};
use util::config::UserConfig;
use util::enforcing_trait_impls::EnforcingSigner;
use util::errors::APIError;
use util::events::{ClosureReason, Event, MessageSendEvent, MessageSendEventsProvider, PaymentPurpose};
...
fn test_insane_channel_opens() {
    ...
    let channel_reserve_satoshis =
        Channel::<EnforcingSigner>::get_holder_selected_channel_reserve_satoshis(channel_value_sat);
    let push_msat = (channel_value_sat - channel_reserve_satoshis) * 1000;

    // Have node0 initiate a channel to node1 with aforementioned parameters
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), channel_value_sat, push_msat, 42, None)
        .unwrap();
        ...
        if let MessageSendEvent::HandleError { ref action, .. } = msg_events[0] {
            match action {
                &ErrorAction::SendErrorMessage { .. } => {
                    nodes[1]
                        .logger
                        .assert_log_regex("lightning::ln::channelmanager".to_string(), expected_regex, 1);
                },
    ...
    insane_open_helper(r"push_msat \d+ was larger than funding value \d+", |mut msg| {
...
fn do_test_counterparty_no_reserve(send_from_initiator: bool) {
    ...
    push_amt -= feerate_per_kw as u64 * (commitment_tx_base_weight(opt_anchors) + 4 * COMMITMENT_TX_WEIGHT_PER_HTLC)
        / 1000
        * 1000;
    ...
    {
        let mut lock;
        let mut chan = get_channel_ref!(
            if send_from_initiator { &nodes[1] } else { &nodes[0] },
            lock,
            temp_channel_id
        );
...
fn test_async_inbound_update_fee() {
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_revoke_and_ack); // deliver (2)
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_revoke_and_ack); // deliver (3)
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_second_revoke);
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_second_revoke); // deliver (6)
    check_added_monitors!(nodes[1], 1);
}
...
fn test_update_fee_unordered_raa() {
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_revoke_msg); // deliver (2)
    check_added_monitors!(nodes[1], 1);
...
fn test_multi_flight_update_fee() {
    ...
    nodes[1]
        .node
        .handle_update_fee(&nodes[0].node.get_our_node_id(), update_msg_1);
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), commitment_signed_1);
    ...
    nodes[1]
        .node
        .handle_update_fee(&nodes[0].node.get_our_node_id(), &update_msg_2);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_revoke_msg);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_revoke_msg);
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_second_update.commitment_signed);
    ...
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &bs_second_commitment);
    let as_second_revoke = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendRevokeAndACK,
        nodes[1].node.get_our_node_id()
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_second_revoke);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 1);
}
...
fn do_test_sanity_on_in_flight_opens(steps: u8) {
    ...
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), 100000, 10001, 42, None)
        .unwrap();
    ...
    nodes[1]
        .node
        .handle_open_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &open_channel);
    ...
    nodes[0]
        .node
        .handle_accept_channel(&nodes[1].node.get_our_node_id(), InitFeatures::known(), &accept_channel);
    ...
    nodes[1]
        .node
        .handle_funding_created(&nodes[0].node.get_our_node_id(), &funding_created);
    {
...
fn test_update_fee_that_funder_cannot_afford() {
    ...
    let bs_channel_reserve_sats =
        Channel::<EnforcingSigner>::get_holder_selected_channel_reserve_satoshis(channel_value);
    ...
    let feerate = ((channel_value - bs_channel_reserve_sats - push_sats) * 1000
        / (commitment_tx_base_weight(opt_anchors)
            + CONCURRENT_INBOUND_HTLC_FEE_BUFFER as u64 * COMMITMENT_TX_WEIGHT_PER_HTLC)) as u32;
    let non_buffer_feerate =
        ((channel_value - bs_channel_reserve_sats - push_sats) * 1000 / commitment_tx_base_weight(opt_anchors)) as u32;
    {
        let mut feerate_lock = chanmon_cfgs[0].fee_estimator.sat_per_kw.lock().unwrap();
    ...
    nodes[0].logger.assert_log(
        "lightning::ln::channel".to_string(),
    ...
    let res = {
        ...
        local_chan_signer
            .sign_counterparty_commitment(&commitment_tx, Vec::new(), &secp_ctx)
            .unwrap()
    };
    ...
    nodes[1]
        .node
        .handle_update_fee(&nodes[0].node.get_our_node_id(), &update_fee);
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &commit_signed_msg);
...
fn test_update_fee_with_fundee_update_add_htlc() {
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &commitment_update.update_add_htlcs[0]);
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &commitment_update.commitment_signed);
    check_added_monitors!(nodes[0], 1);
...
fn test_update_fee() {
    ...
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &commitment_signed_0);
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &revoke_msg_0);
    let commitment_update = get_htlc_update_msgs!(nodes[1], nodes[0].node.get_our_node_id());
...
fn fake_network_test() {
    ...
    nodes[0].logger.assert_log_regex(
        "lightning::ln::channelmanager".to_string(),
        regex::Regex::new(
            r"Cannot send value that would put us over the max HTLC value in flight our peer will accept \(\d+\)",
        )
        .unwrap(),
        1,
    );
...
fn holding_cell_htlc_counting() {
    ...
    let (route, payment_hash_2, _, payment_secret_2) = get_route_and_payment_hash!(nodes[0], nodes[2], 100000);
    {
        nodes[0]
            .node
            .send_payment(&route, payment_hash_2, &Some(payment_secret_2))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    }
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &bs_fail_updates.update_fail_htlcs[0]);
    ...
    nodes[2]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &initial_payment_event.commitment_msg);
    check_added_monitors!(nodes[2], 1);

    let (bs_revoke_and_ack, bs_commitment_signed) = get_revoke_commit_msgs!(nodes[2], nodes[1].node.get_our_node_id());
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[2].node.get_our_node_id(), &bs_revoke_and_ack);
    check_added_monitors!(nodes[1], 1);
    let as_updates = get_htlc_update_msgs!(nodes[1], nodes[2].node.get_our_node_id());

    nodes[1]
        .node
        .handle_commitment_signed(&nodes[2].node.get_our_node_id(), &bs_commitment_signed);
    check_added_monitors!(nodes[1], 1);
    ...
    for ref update in as_updates.update_add_htlcs.iter() {
        nodes[2]
            .node
            .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), update);
    }
    nodes[2]
        .node
    ...
    nodes[2]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &as_raa);
    check_added_monitors!(nodes[2], 1);
    let (bs_revoke_and_ack, bs_commitment_signed) = get_revoke_commit_msgs!(nodes[2], nodes[1].node.get_our_node_id());
    ...
    nodes[2]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &as_final_raa);
    check_added_monitors!(nodes[2], 1);
...
fn duplicate_htlc_test() {
    ...
    *nodes[0].network_payment_count.borrow_mut() -= 1;
    assert_eq!(
        route_payment(&nodes[1], &vec!(&nodes[3])[..], 1000000).0,
        payment_preimage
    );
...
fn test_duplicate_htlc_different_direction_onchain() {
    ...
    assert_eq!(
        claim_txn[0].input[0].previous_output,
        claim_txn[3].input[0].previous_output
    );

    assert_eq!(claim_txn[0].input.len(), 1);
    assert_eq!(
        claim_txn[0].input[0].witness.last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    ); // HTLC 1 <--> 0, preimage tx
    check_spends!(claim_txn[0], remote_txn[0]);
    assert_eq!(
        remote_txn[0].output[claim_txn[0].input[0].previous_output.vout as usize].value,
        800
    );
    assert_eq!(claim_txn[6].input.len(), 1);
    assert_eq!(
        claim_txn[6].input[0].witness.last().unwrap().len(),
        ACCEPTED_HTLC_SCRIPT_WEIGHT
    ); // HTLC 0 <--> 1, timeout tx
    check_spends!(claim_txn[6], remote_txn[0]);
    assert_eq!(
        remote_txn[0].output[claim_txn[6].input[0].previous_output.vout as usize].value,
        900
    );
...
fn test_basic_channel_reserve() {
    ...
    let commit_tx_fee = 2 * commit_tx_fee_msat(
        get_feerate!(nodes[0], chan.2),
        1 + 1,
        get_opt_anchors!(nodes[0], chan.2),
    );
    let max_can_send = 5000000 - channel_reserve - commit_tx_fee;
    let (route, our_payment_hash, _, our_payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[1], max_can_send + 1);
    let err = nodes[0]
        .node
        .send_payment(&route, our_payment_hash, &Some(our_payment_secret))
        .err()
    ...
    nodes[0].logger.assert_log_contains(
        "lightning::ln::channelmanager".to_string(),
        "Cannot send value that would put our balance under counterparty-announced channel reserve value".to_string(),
        1,
    );
...
fn test_fee_spike_violation_fails_htlc() {
    ...
    let (onion_payloads, htlc_msat, htlc_cltv) =
        onion_utils::build_onion_payloads(&route.paths[0], 3460001, &Some(payment_secret), cur_height, &None).unwrap();
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(&nodes[0].node.get_our_node_id(), &msg);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &raa_msg);
...
fn test_chan_reserve_violation_outbound_htlc_inbound_chan() {
    ...
    nodes[1].logger.assert_log(
        "lightning::ln::channelmanager".to_string(),
        "Cannot send value that would put counterparty balance under holder-announced channel reserve value"
            .to_string(),
        1,
    );
}
...
fn test_chan_reserve_violation_inbound_htlc_outbound_channel() {
    ...
    let onion_keys = onion_utils::construct_onion_keys(&secp_ctx, &route.paths[0], &session_priv).unwrap();
    let (onion_payloads, htlc_msat, htlc_cltv) =
        onion_utils::build_onion_payloads(&route.paths[0], 700_000, &Some(payment_secret), cur_height, &None).unwrap();
    let onion_packet = onion_utils::construct_onion_packet(onion_payloads, onion_keys, [0; 32], &payment_hash);
    let msg = msgs::UpdateAddHTLC {
        channel_id: chan.2,
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &msg);
    // Check that the payment failed and the channel is closed in response to the malicious UpdateAdd.
    nodes[0].logger.assert_log(
        "lightning::ln::channelmanager".to_string(),
        "Cannot accept HTLC that would put our balance under counterparty-announced channel reserve value".to_string(),
        1,
    );
    ...
    check_closed_event!(
        ...
        1,
        ClosureReason::ProcessingError {
            err: "Cannot accept HTLC that would put our balance under counterparty-announced channel reserve value"
                .to_string()
        }
    );
}
...
fn test_chan_reserve_dust_inbound_htlcs_outbound_chan() {
    ...
    let dust_amt = crate::ln::channel::MIN_CHAN_DUST_LIMIT_SATOSHIS * 1000
        + feerate_per_kw as u64 * htlc_success_tx_weight(opt_anchors) / 1000 * 1000
        - 1;
    ...
    let (route, our_payment_hash, _, our_payment_secret) =
        get_route_and_payment_hash!(nodes[1], nodes[0], dust_amt + 1);
    unwrap_send_err!(nodes[1].node.send_payment(&route, our_payment_hash, &Some(our_payment_secret)), true, APIError::ChannelUnavailable { ref err },
		assert_eq!(err, "Cannot send value that would put counterparty balance under holder-announced channel reserve value"));
}
...
fn test_chan_init_feerate_unaffordability() {
    ...
    assert_eq!(
        nodes[0]
            .node
            .create_channel(nodes[1].node.get_our_node_id(), 100_000, push_amt + 1, 42, None)
            .unwrap_err(),
        APIError::APIMisuseError {
            err: "Funding amount (356) can't even pay fee for initial commitment transaction fee of 357.".to_string()
        }
    );
    ...
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), 100_000, push_amt, 42, None)
        .unwrap();
...
fn test_chan_reserve_violation_inbound_htlc_inbound_chan() {
    ...
    let recv_value_1 =
        (chan_stat.value_to_self_msat - chan_stat.channel_reserve_msat - total_routing_fee_msat - commit_tx_fee_2_htlc)
            / 2;
    ...
    let payment_event_1 = {
        nodes[0]
            .node
            .send_payment(&route_1, our_payment_hash_1, &Some(our_payment_secret_1))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    ...
    let (onion_payloads, htlc_msat, htlc_cltv) =
        onion_utils::build_onion_payloads(&route_2.paths[0], recv_value_2, &None, cur_height, &None).unwrap();
    ...
    nodes[1].logger.assert_log(
        "lightning::ln::channelmanager".to_string(),
        "Remote HTLC add would put them under remote reserve value".to_string(),
        1,
    );
...
fn test_channel_reserve_holding_cell_htlcs() {
    ...
    // attempt to send amt_msat > their_max_htlc_value_in_flight_msat
    {
        let (mut route, our_payment_hash, _, our_payment_secret) =
            get_route_and_payment_hash!(nodes[0], nodes[2], recv_value_0);
    ...
    loop {
        ...
        if stat01.value_to_self_msat
            < stat01.channel_reserve_msat + commit_tx_fee_all_htlcs + ensure_htlc_amounts_above_dust_buffer + amt_msat
        {
            break;
        }
    ...
    let recv_value_1 =
        (stat01.value_to_self_msat - stat01.channel_reserve_msat - total_fee_msat - commit_tx_fee_2_htlcs) / 2;
    ...
    let recv_value_2 =
        stat01.value_to_self_msat - amt_msat_1 - stat01.channel_reserve_msat - total_fee_msat - commit_tx_fee_2_htlcs;
    {
        let (route, our_payment_hash, _, our_payment_secret) =
            get_route_and_payment_hash!(nodes[0], nodes[2], recv_value_2 + 1);
        unwrap_send_err!(nodes[0].node.send_payment(&route, our_payment_hash, &Some(our_payment_secret)), true, APIError::ChannelUnavailable { ref err },
			assert!(regex::Regex::new(r"Cannot send value that would put our balance under counterparty-announced channel reserve value \(\d+\)").unwrap().is_match(err)));
        assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    }
    ...
    nodes[0]
        .node
        .send_payment(&route_21, our_payment_hash_21, &Some(our_payment_secret_21))
        .unwrap();
    check_added_monitors!(nodes[0], 0);
    let events = nodes[0].node.get_and_clear_pending_events();
    assert_eq!(events.len(), 0);
    ...
    {
        let (route, our_payment_hash, _, our_payment_secret) =
            get_route_and_payment_hash!(nodes[0], nodes[2], recv_value_22 + 1);
        unwrap_send_err!(nodes[0].node.send_payment(&route, our_payment_hash, &Some(our_payment_secret)), true, APIError::ChannelUnavailable { ref err },
			assert!(regex::Regex::new(r"Cannot send value that would put our balance under counterparty-announced channel reserve value \(\d+\)").unwrap().is_match(err)));
        assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
        nodes[0].logger.assert_log_contains(
    ...
    nodes[0]
        .node
        .send_payment(&route_22, our_payment_hash_22, &Some(our_payment_secret_22))
        .unwrap();
    check_added_monitors!(nodes[0], 0);
    assert!(nodes[0].node.get_and_clear_pending_events().is_empty());
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &payment_event_1.commitment_msg);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &as_revoke_and_ack);
    check_added_monitors!(nodes[0], 1);
    let commitment_update_2 = get_htlc_update_msgs!(nodes[0], nodes[1].node.get_our_node_id());

    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &as_commitment_signed);
    let bs_revoke_and_ack = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendRevokeAndACK,
        nodes[1].node.get_our_node_id()
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &bs_revoke_and_ack);
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    check_added_monitors!(nodes[1], 1);
...
fn channel_reserve_in_flight_removes() {
    ...
    let (payment_preimage_1, _, _) = route_payment(
        &nodes[0],
        &[&nodes[1]],
    ...
    let send_1 = {
        nodes[0]
            .node
            .send_payment(&route, payment_hash_3, &Some(payment_secret_3))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
        let mut events = nodes[0].node.get_and_clear_pending_msg_events();
        assert_eq!(events.len(), 1);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &bs_removes.update_fulfill_htlcs[0]);
    nodes[0]
        .node
    ...
    nodes[1]
        .node
        .handle_update_add_htlc(&nodes[0].node.get_our_node_id(), &send_1.msgs[0]);
    nodes[1]
        .node
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_raa);
    check_added_monitors!(nodes[1], 1);
    let bs_cs = get_htlc_update_msgs!(nodes[1], nodes[0].node.get_our_node_id());

    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &bs_raa);
    check_added_monitors!(nodes[0], 1);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &bs_cs.update_fulfill_htlcs[0]);
    nodes[0]
        .node
    ...
    let send_2 = {
        nodes[1]
            .node
            .send_payment(&route, payment_hash_4, &Some(payment_secret_4))
            .unwrap();
        check_added_monitors!(nodes[1], 1);
        let mut events = nodes[1].node.get_and_clear_pending_msg_events();
        assert_eq!(events.len(), 1);
    ...
    nodes[0]
        .node
        .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &send_2.msgs[0]);
    nodes[0]
        .node
...
fn channel_monitor_network_test() {
    ...
    let chan_3_mon = nodes[3].chain_monitor.chain_monitor.remove_monitor(&OutPoint {
        txid: chan_3.3.txid(),
        index: 0,
    });
    ...
    nodes[3]
        ...
        .handle_channel_update(&close_chan_update_2)
        .unwrap();
    nodes[4]
        .net_graph_msg_handler
...
fn test_htlc_on_chain_success() {
    ...
    let (our_payment_preimage, payment_hash_1, _payment_secret) =
        route_payment(&nodes[0], &vec![&nodes[1], &nodes[2]], 3000000);
    let (our_payment_preimage_2, payment_hash_2, _payment_secret_2) =
        route_payment(&nodes[0], &vec![&nodes[1], &nodes[2]], 3000000);
    ...
    let commitment_spend = if node_txn[0].input[0].previous_output.txid == node_a_commitment_tx[0].txid() {
        check_spends!(node_txn[1], commitment_tx[0]);
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![node_a_commitment_tx[0].clone(), commitment_spend.clone()],
        },
    );
...
fn do_test_htlc_on_chain_timeout(connect_style: ConnectStyle) {
    ...
    *nodes[2].connect_style.borrow_mut() = connect_style;
    ...
    let (_payment_preimage, payment_hash, _payment_secret) =
        route_payment(&nodes[0], &vec![&nodes[1], &nodes[2]], 3000000);
    ...
    let timeout_tx;
    {
        ...
        check_spends!(node_txn[2], commitment_tx[0]);
        assert_eq!(
            node_txn[2].clone().input[0].witness.last().unwrap().len(),
            ACCEPTED_HTLC_SCRIPT_WEIGHT
        );
        ...
        assert_eq!(
            node_txn[1].clone().input[0].witness.last().unwrap().len(),
            OFFERED_HTLC_SCRIPT_WEIGHT
        );
...
fn do_test_commitment_revoked_fail_backward_exhaustive(deliver_bs_raa: bool, use_dust: bool, no_to_remote: bool) {
    ...
    let (payment_preimage, _payment_hash, _payment_secret) = route_payment(
        &nodes[0],
        &[&nodes[1], &nodes[2]],
    ...
    nodes[1]
        .node
        .send_payment(&route, fourth_payment_hash, &Some(fourth_payment_secret))
        .unwrap();
    assert!(nodes[1].node.get_and_clear_pending_msg_events().is_empty());
    ...
    if deliver_bs_raa {
        nodes[1]
            .node
            .handle_revoke_and_ack(&nodes[2].node.get_our_node_id(), &bs_raa);
    ...
    match events[if deliver_bs_raa { 2 } else { 1 }] {
        MessageSendEvent::HandleError {
            action:
                ErrorAction::SendErrorMessage {
                    msg: msgs::ErrorMessage { channel_id, ref data },
                },
            node_id: _,
        } => {
...
fn fail_backward_pending_htlc_upon_channel_failure() {
        ...
        get_route_and_payment_hash!(nodes[0], nodes[1], 50_000);
    {
        nodes[0]
            .node
            .send_payment(&route, failed_payment_hash, &Some(failed_payment_secret))
            .unwrap();
        check_added_monitors!(nodes[0], 0);
    ...
    // Alice <- Bob: Send a malformed update_add_htlc so Alice fails the channel.
    {
        ...
        let (onion_payloads, _amount_msat, cltv_expiry) =
            onion_utils::build_onion_payloads(&route.paths[0], 50_000, &Some(payment_secret), current_height, &None)
                .unwrap();
        let onion_keys = onion_utils::construct_onion_keys(&secp_ctx, &route.paths[0], &session_priv).unwrap();
        ...
        nodes[0]
            .node
            .handle_update_add_htlc(&nodes[1].node.get_our_node_id(), &update_add_htlc);
    }
    let events = nodes[0].node.get_and_clear_pending_events();
    assert_eq!(events.len(), 2);
...
fn test_htlc_ignore_latest_remote_commitment() {
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![node_txn[0].clone(), node_txn[1].clone()],
        },
    );
    check_closed_broadcast!(nodes[1], true);
    check_added_monitors!(nodes[1], 1);
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![node_txn[0].clone(), node_txn[2].clone()],
        },
    );
}
...
fn test_force_close_fail_back() {
    ...
    nodes[2]
        .node
        .force_close_channel(&payment_event.commitment_msg.channel_id)
        .unwrap();
    check_closed_broadcast!(nodes[2], true);
    check_added_monitors!(nodes[2], 1);
...
fn test_dup_events_on_peer_disconnect() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &claim_msgs.update_fulfill_htlcs[0]);
...
fn do_test_drop_messages_peer_disconnect(messages_delivered: u8, simulate_broken_lnd: bool) {
    ...
    if messages_delivered == 0 {
        let (funding_locked, _, _) = create_chan_between_nodes_with_value_a(
            &nodes[0],
            &nodes[1],
            100000,
            10001,
    ...
    let payment_event = {
        nodes[0]
            .node
            .send_payment(&route, payment_hash_1, &Some(payment_secret_1))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
    ...
    if messages_delivered < 3 {
        if simulate_broken_lnd {
            ...
            nodes[1]
                .node
                .handle_funding_locked(&nodes[0].node.get_our_node_id(), &as_funding_locked.as_ref().unwrap().0);
        }
        ...
        if messages_delivered >= 2 {
            ...
            if messages_delivered >= 3 {
                ...
                if messages_delivered >= 4 {
                    nodes[1]
                        .node
                        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_commitment_signed);
                    let bs_revoke_and_ack = get_event_msg!(
                        nodes[1],
                        MessageSendEvent::SendRevokeAndACK,
                        nodes[0].node.get_our_node_id()
...
fn test_funding_peer_disconnect() {
    ...
    let bs_reestablish = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendChannelReestablish,
        nodes[0].node.get_our_node_id()
    );
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &bs_reestablish);
    ...
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &as_reestablish);
    ...
    nodes[1]
        .node
        .handle_funding_locked(&nodes[0].node.get_our_node_id(), &as_funding_locked);
    ...
    nodes[0]
        .node
        .handle_funding_locked(&nodes[1].node.get_our_node_id(), &bs_funding_locked);
    ...
    nodes[0]
        .net_graph_msg_handler
        .handle_channel_announcement(&chan_announcement)
        .unwrap();
    nodes[0]
        .net_graph_msg_handler
        .handle_channel_update(&bs_update)
        .unwrap();
    nodes[0]
        .net_graph_msg_handler
        .handle_channel_update(&as_update)
        .unwrap();
    ...
    get_monitor!(nodes[0], chan_id)
        .write(&mut chan_0_monitor_serialized)
        .unwrap();
    ...
    let (_, mut chan_0_monitor) =
        <(BlockHash, ChannelMonitor<EnforcingSigner>)>::read(&mut chan_0_monitor_read, keys_manager).unwrap();
    ...
    assert!(nodes[0]
        .chain_monitor
        .watch_channel(chan_0_monitor.get_funding_txo().0, chan_0_monitor)
        .is_ok());
...
fn test_funding_locked_without_best_block_updated() {
    ...
    *nodes[0].connect_style.borrow_mut() = ConnectStyle::BestBlockFirstSkippingBlocks;
    ...
    let funding_tx = create_chan_between_nodes_with_value_init(
        &nodes[0],
        &nodes[1],
        1_000_000,
        0,
...
fn test_drop_messages_peer_disconnect_dual_htlc() {
    ...
    match events_2[0] {
        MessageSendEvent::UpdateHTLCs {
            ...
            nodes[0]
                .node
                .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &update_fulfill_htlcs[0]);
...
fn do_test_holding_cell_htlc_add_timeouts(forwarded_htlc: bool) {
    ...
    let (route, first_payment_hash, _, first_payment_secret) = get_route_and_payment_hash!(nodes[1], nodes[2], 100000);
    {
        nodes[1]
            .node
            .send_payment(&route, first_payment_hash, &Some(first_payment_secret))
            .unwrap();
    }
    ...
    sending_node
        .node
        .send_payment(&route, second_payment_hash, &Some(second_payment_secret))
        .unwrap();
    if forwarded_htlc {
        check_added_monitors!(nodes[0], 1);
...
fn test_no_txn_manager_serialize_deserialize() {
    ...
    for node in nodes.iter() {
        assert!(node
            .net_graph_msg_handler
            .handle_channel_announcement(&announcement)
            .unwrap());
...
fn test_manager_serialize_deserialize_events() {
    ...
    node_a
        .node
        .create_channel(node_b.node.get_our_node_id(), channel_value, push_msat, 42, None)
        .unwrap();
    ...
    let bs_funding_signed = get_event_msg!(
        node_b,
        MessageSendEvent::SendFundingSigned,
        node_a.node.get_our_node_id()
    );
    node_a
        .node
        .handle_funding_signed(&node_b.node.get_our_node_id(), &bs_funding_signed);
    {
    ...
    let nodes_0_serialized = nodes[0].node.encode();
    let mut chan_0_monitor_serialized = test_utils::TestVecWriter(Vec::new());
    get_monitor!(nodes[0], bs_funding_signed.channel_id)
        .write(&mut chan_0_monitor_serialized)
        .unwrap();
...
fn test_manager_serialize_deserialize_inconsistent_monitor() {
    ...
    for monitor in node_0_monitors.drain(..) {
        assert!(nodes[0]
            .chain_monitor
            .watch_channel(monitor.get_funding_txo().0, monitor)
            .is_ok());
        check_added_monitors!(nodes[0], 1);
    }
    nodes[0].node = &nodes_0_deserialized;
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[3].node.get_our_node_id(), &reestablish);
...
fn test_static_spendable_outputs_justice_tx_revoked_htlc_timeout_tx() {
    ...
    assert_eq!(
        revoked_htlc_txn[1].input[0].witness.last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    );
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![revoked_local_txn[0].clone(), revoked_htlc_txn[1].clone()],
        },
    );
    check_closed_broadcast!(nodes[1], true);
    check_added_monitors!(nodes[1], 1);
...
fn test_static_spendable_outputs_justice_tx_revoked_htlc_success_tx() {
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![revoked_local_txn[0].clone(), revoked_htlc_txn[0].clone()],
        },
    );
    check_closed_broadcast!(nodes[0], true);
    check_added_monitors!(nodes[0], 1);
...
fn test_onchain_to_onchain_claim() {
    ...
    assert_eq!(
        c_txn[2].input[0].witness.clone().last().unwrap().len(),
        ACCEPTED_HTLC_SCRIPT_WEIGHT
    );
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![c_txn[1].clone(), c_txn[2].clone()],
        },
    );
    check_added_monitors!(nodes[1], 1);
    let events = nodes[1].node.get_and_clear_pending_events();
...
fn test_duplicate_payment_hash_one_failure_one_success() {
    ...
    let (our_payment_preimage, duplicate_payment_hash, _) =
        route_payment(&nodes[0], &vec![&nodes[1], &nodes[2]][..], 900000);

    let payment_secret = nodes[3]
        .node
        .create_inbound_payment_for_hash(duplicate_payment_hash, None, 7200)
        .unwrap();
    ...
    let htlc_timeout_tx;
    {
        ...
        assert_eq!(
            node_txn[1].input[0].previous_output,
            node_txn[2].input[0].previous_output
        );
        check_spends!(node_txn[3], commitment_txn[0]);
        assert_ne!(
            node_txn[1].input[0].previous_output,
            node_txn[3].input[0].previous_output
        );
        ...
        assert_eq!(
            node_txn[3].input[0].witness.last().unwrap().len(),
            ACCEPTED_HTLC_SCRIPT_WEIGHT
        );
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &htlc_updates.update_fail_htlcs[0]);
    assert!(nodes[0].node.get_and_clear_pending_msg_events().is_empty());
    {
...
fn do_test_fail_backwards_unrevoked_remote_announce(deliver_last_raa: bool, announce_latest: bool) {
    ...
    send_along_route_with_secret(
        ...
        nodes[5]
            .node
            .create_inbound_payment_for_hash(payment_hash_1, None, 7200)
            .unwrap(),
    ...
    send_along_route_with_secret(
        ...
        nodes[5]
            .node
            .create_inbound_payment_for_hash(payment_hash_2, None, 7200)
            .unwrap(),
    ); // not added < dust limit + HTLC tx fee
       // 4th HTLC:
    ...
    send_along_route_with_secret(
        ...
        nodes[5]
            .node
            .create_inbound_payment_for_hash(payment_hash_3, None, 7200)
            .unwrap(),
    );
    // 7th HTLC:
    send_along_route_with_secret(
        ...
        nodes[5]
            .node
            .create_inbound_payment_for_hash(payment_hash_4, None, 7200)
            .unwrap(),
    );
    ...
    send_along_route_with_secret(
        ...
        nodes[5]
            .node
            .create_inbound_payment_for_hash(payment_hash_5, None, 7200)
            .unwrap(),
    ); // not added < dust limit + HTLC tx fee
    ...
    send_along_route_with_secret(
        ...
        nodes[5]
            .node
            .create_inbound_payment_for_hash(payment_hash_6, None, 7200)
            .unwrap(),
    );
    ...
    nodes[3]
        .node
        .handle_update_fail_htlc(&nodes[4].node.get_our_node_id(), &four_removes.update_fail_htlcs[0]);
    nodes[3]
        .node
        .handle_update_fail_htlc(&nodes[4].node.get_our_node_id(), &four_removes.update_fail_htlcs[1]);
    nodes[3]
        .node
        .handle_update_fail_htlc(&nodes[4].node.get_our_node_id(), &four_removes.update_fail_htlcs[2]);
    nodes[3]
        .node
        .handle_update_fail_htlc(&nodes[4].node.get_our_node_id(), &four_removes.update_fail_htlcs[3]);
    ...
    nodes[3]
        .node
        .handle_update_fail_htlc(&nodes[5].node.get_our_node_id(), &two_removes.update_fail_htlcs[0]);
    nodes[3]
        .node
        .handle_update_fail_htlc(&nodes[5].node.get_our_node_id(), &two_removes.update_fail_htlcs[1]);
    ...
    nodes[2]
        .node
        .handle_update_fail_htlc(&nodes[3].node.get_our_node_id(), &six_removes.update_fail_htlcs[0]);
    nodes[2]
        .node
        .handle_update_fail_htlc(&nodes[3].node.get_our_node_id(), &six_removes.update_fail_htlcs[1]);
    nodes[2]
        .node
        .handle_update_fail_htlc(&nodes[3].node.get_our_node_id(), &six_removes.update_fail_htlcs[2]);
    nodes[2]
        .node
        .handle_update_fail_htlc(&nodes[3].node.get_our_node_id(), &six_removes.update_fail_htlcs[3]);
    nodes[2]
        .node
        .handle_update_fail_htlc(&nodes[3].node.get_our_node_id(), &six_removes.update_fail_htlcs[4]);
    nodes[2]
        .node
        .handle_update_fail_htlc(&nodes[3].node.get_our_node_id(), &six_removes.update_fail_htlcs[5]);
    if deliver_last_raa {
    ...
    for msg in cs_msgs {
        match msg {
            MessageSendEvent::UpdateHTLCs {
                ...
                target
                    .node
                    .handle_update_fail_htlc(&nodes[2].node.get_our_node_id(), &updates.update_fail_htlcs[1]);
                target
                    .node
                    .handle_update_fail_htlc(&nodes[2].node.get_our_node_id(), &updates.update_fail_htlcs[2]);
                if announce_latest {
                    target
                        .node
...
fn test_dynamic_spendable_outputs_local_htlc_timeout_tx() {
    ...
    assert!(
        spend_txn[2].input[0].sequence == BREAKDOWN_TIMEOUT as u32
...
fn test_key_derivation_params() {
    ...
    let node = NodeCfg {
        ...
        network_graph: &chanmon_cfgs[0].network_graph,
        node_seed: seed,
    ...
    let (from_0_funding_key_0, from_0_funding_key_1) = (
        PublicKey::from_slice(&local_txn_0[0].input[0].witness[3][2..35]),
        PublicKey::from_slice(&local_txn_0[0].input[0].witness[3][36..69]),
    );
    let (from_1_funding_key_0, from_1_funding_key_1) = (
        PublicKey::from_slice(&local_txn_1[0].input[0].witness[3][2..35]),
        PublicKey::from_slice(&local_txn_1[0].input[0].witness[3][36..69]),
    );
...
fn do_htlc_claim_local_commitment_only(use_dust: bool) {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &bs_updates.update_fulfill_htlcs[0]);
    expect_payment_sent_without_paths!(nodes[0], payment_preimage);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &as_updates.0);
    check_added_monitors!(nodes[1], 1);
    ...
    let mut block = Block {
        header: BlockHeader {
            version: 0x20000000,
            prev_blockhash: starting_block.0,
            merkle_root: Default::default(),
            time: 42,
            bits: 42,
            nonce: 42,
...
fn do_htlc_claim_previous_remote_commitment_only(use_dust: bool, check_revoke_no_close: bool) {
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[0].node.get_our_node_id(), &as_updates.1);
    check_added_monitors!(nodes[1], 1);
    let bs_revoke_and_ack = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendRevokeAndACK,
...
fn test_fail_holding_cell_htlc_upon_free() {
    ...
    nodes[0]
        .logger
        .assert_log("lightning::ln::channel".to_string(), failure_log.to_string(), 1);
...
fn test_free_and_fail_holding_cell_htlcs() {
    ...
    nodes[0]
        .node
        .send_payment(&route_1, payment_hash_1, &Some(payment_secret_1))
        .unwrap();
    chan_stat = get_channel_value_stat!(nodes[0], chan.2);
    assert_eq!(chan_stat.holding_cell_outbound_amount_msat, amt_1);
    let payment_id_2 = nodes[0]
        .node
        .send_payment(&route_2, payment_hash_2, &Some(payment_secret_2))
        .unwrap();
    chan_stat = get_channel_value_stat!(nodes[0], chan.2);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &revoke_and_ack);
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &commitment_signed);
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[0].node.get_our_node_id(), &raa);
    check_added_monitors!(nodes[1], 1);
    nodes[1]
        .node
        .handle_update_add_htlc(&nodes[0].node.get_our_node_id(), &payment_event.msgs[0]);
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &update_msgs.update_fulfill_htlcs[0]);
...
fn test_fail_holding_cell_htlc_upon_free_multihop() {
    ...
    let chan_1_2 = create_announced_chan_between_nodes_with_value(
        &nodes,
        1,
        2,
        100000,
    ...
    nodes[2]
        .node
        .handle_update_fee(&nodes[1].node.get_our_node_id(), update_msg.unwrap());
    ...
    let total_routing_fee_msat = (nodes.len() - 2) as u64 * feemsat;
    let max_can_send =
        5000000 - channel_reserve - 2 * commit_tx_fee_msat(feerate, 1 + 1, opt_anchors) - total_routing_fee_msat;
    let (route, our_payment_hash, _, our_payment_secret) =
    ...
    nodes[1]
        .node
        .handle_revoke_and_ack(&nodes[2].node.get_our_node_id(), &raa);
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[2].node.get_our_node_id(), &commitment_signed);
    check_added_monitors!(nodes[1], 2);
    ...
    nodes[2]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &raa_msg);
    check_added_monitors!(nodes[2], 1);
    ...
    nodes[0]
        .node
        .handle_revoke_and_ack(&nodes[1].node.get_our_node_id(), &raa);
    expect_payment_failed_with_update!(
        nodes[0],
        our_payment_hash,
        false,
...
fn test_update_add_htlc_bolt2_sender_zero_value_msat() {
    ...
    nodes[0].logger.assert_log_contains(
        "lightning::ln::channelmanager".to_string(),
        "Cannot send 0-msat HTLC".to_string(),
        1,
    );
}
...
fn test_update_add_htlc_bolt2_sender_cltv_expiry_too_high() {
    ...
    let (mut route, our_payment_hash, _, our_payment_secret) =
        get_route_and_payment_hash!(nodes[0], nodes[1], vec![], 100000000, 0);
...
fn test_update_add_htlc_bolt2_receiver_check_amount_received_more_than_min() {
    ...
    assert!(regex::Regex::new(
        r"Remote side tried to send less than our minimum HTLC value\. Lower limit: \(\d+\)\. Actual: \(\d+\)"
    )
    .unwrap()
...
fn test_update_add_htlc_bolt2_receiver_check_max_htlc_limit() {
    ...
    let (onion_payloads, _htlc_msat, htlc_cltv) =
        onion_utils::build_onion_payloads(&route.paths[0], 3999999, &Some(our_payment_secret), cur_height, &None)
            .unwrap();
...
fn test_update_add_htlc_bolt2_receiver_check_max_in_flight_msat() {
    ...
    updates.update_add_htlcs[0].amount_msat =
        get_channel_value_stat!(nodes[1], chan.2).counterparty_max_htlc_value_in_flight_msat + 1;
    nodes[1]
        .node
        .handle_update_add_htlc(&nodes[0].node.get_our_node_id(), &updates.update_add_htlcs[0]);
...
fn test_update_fulfill_htlc_bolt2_update_fulfill_htlc_before_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &update_msg);

    assert!(nodes[0].node.list_channels().is_empty());
    let err_msg = check_closed_broadcast!(nodes[0], true).unwrap();
    assert!(
        regex::Regex::new(r"Remote tried to fulfill/fail HTLC \(\d+\) before it had been committed")
            .unwrap()
            .is_match(err_msg.data.as_str())
    );
    check_added_monitors!(nodes[0], 1);
...
fn test_update_fulfill_htlc_bolt2_update_fail_htlc_before_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &update_msg);
...
fn test_update_fulfill_htlc_bolt2_update_fail_malformed_htlc_before_commitment() {
    ...
    nodes[0]
        .node
        .handle_update_fail_malformed_htlc(&nodes[1].node.get_our_node_id(), &update_msg);
...
fn test_update_fulfill_htlc_bolt2_incorrect_htlc_id() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &update_fulfill_msg);
...
fn do_test_sweep_outbound_htlc_failure_update(revoked: bool, local: bool) {
    ...
    let (_payment_preimage_1, dust_hash, _payment_secret_1) =
...
fn test_user_configurable_csv_delay() {
    ...
    nodes[1]
        .node
        .create_channel(nodes[0].node.get_our_node_id(), 1000000, 1000000, 42, None)
        .unwrap();
    ...
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), 1000000, 1000000, 42, None)
        .unwrap();
    nodes[1].node.handle_open_channel(
        &nodes[0].node.get_our_node_id(),
        InitFeatures::known(),
...
fn test_data_loss_protect() {
    ...
    get_monitor!(nodes[0], chan.2)
        .write(&mut previous_chain_monitor_state)
        .unwrap();
    ...
    let mut chain_monitor = <(BlockHash, ChannelMonitor<EnforcingSigner>)>::read(
        &mut io::Cursor::new(previous_chain_monitor_state.0),
        keys_manager,
    )
    .unwrap()
    .1;
    ...
    persister = test_utils::TestPersister::new();
    monitor = test_utils::TestChainMonitor::new(
        Some(&chain_source),
        &tx_broadcaster,
        &logger,
        &fee_estimator,
    ...
    node_state_0 = {
        ...
        <(
            ...
            &mut io::Cursor::new(previous_node_state),
            ChannelManagerReadArgs {
                keys_manager,
                fee_estimator: &fee_estimator,
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &reestablish_0[0]);
    check_added_monitors!(nodes[0], 1);
    ...
    check_closed_event!(
        ...
        1,
        ClosureReason::ProcessingError {
            err: "Peer attempted to reestablish channel with a very old local commitment transaction".to_string()
        }
    );
    assert_eq!(
...
fn test_check_htlc_underpaying() {
    ...
    let payment_params =
        PaymentParameters::from_node_id(nodes[1].node.get_our_node_id()).with_features(InvoiceFeatures::known());
    let route = get_route(
        &nodes[0].node.get_our_node_id(),
        &payment_params,
    ...
    let our_payment_secret = nodes[1]
        .node
        .create_inbound_payment_for_hash(our_payment_hash, Some(100_000), 7200)
        .unwrap();
    nodes[0]
        .node
        .send_payment(&route, our_payment_hash, &Some(our_payment_secret))
...
fn test_announce_disable_channels() {
    ...
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &reestablish_2[1]);
    handle_chan_reestablish_msgs!(nodes[0], nodes[1]);
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &reestablish_1[1]);
    handle_chan_reestablish_msgs!(nodes[1], nodes[0]);
    // Reestablish chan_3
    nodes[0]
        .node
        .handle_channel_reestablish(&nodes[1].node.get_our_node_id(), &reestablish_2[2]);
    handle_chan_reestablish_msgs!(nodes[0], nodes[1]);
    nodes[1]
        .node
        .handle_channel_reestablish(&nodes[0].node.get_our_node_id(), &reestablish_1[2]);
    handle_chan_reestablish_msgs!(nodes[1], nodes[0]);
...
fn test_bump_penalty_txn_on_revoked_commitment() {
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![revoked_txn[0].clone()],
        },
    );
    check_added_monitors!(nodes[1], 1);
...
fn test_bump_penalty_txn_on_revoked_htlcs() {
    ...
    let payment_params =
        PaymentParameters::from_node_id(nodes[0].node.get_our_node_id()).with_features(InvoiceFeatures::known());
    let route = get_route(
        &nodes[1].node.get_our_node_id(),
        &payment_params,
    ...
    connect_block(
        ...
        &Block {
            header,
            txdata: vec![revoked_local_txn[0].clone()],
        },
    );
    check_closed_broadcast!(nodes[1], true);
    check_added_monitors!(nodes[1], 1);
    ...
    assert_eq!(
        revoked_htlc_txn[2].input[0].witness.last().unwrap().len(),
        OFFERED_HTLC_SCRIPT_WEIGHT
    );
    ...
    let header_11 = BlockHeader {
        version: 0x20000000,
        prev_blockhash: hash_128,
        merkle_root: Default::default(),
        time: 42,
        bits: 42,
        nonce: 42,
    ...
    let header_129 = BlockHeader {
        version: 0x20000000,
        prev_blockhash: header_11.block_hash(),
        merkle_root: Default::default(),
        time: 42,
        bits: 42,
        nonce: 42,
    ...
    connect_block(
        ...
        &Block {
            header: header_129,
            txdata: vec![revoked_htlc_txn[0].clone(), revoked_htlc_txn[2].clone()],
        },
    );
    let events = nodes[0].node.get_and_clear_pending_events();
    expect_pending_htlcs_forwardable_from_events!(nodes[0], events[0..1], true);
    ...
    let penalty_txn;
    {
        ...
        assert_eq!(
            node_txn[1].input[0].previous_output,
            revoked_htlc_txn[2].input[0].previous_output
        );
        ...
        let fee_1 =
            revoked_htlc_txn[0].output[0].value + revoked_htlc_txn[2].output[0].value - node_txn[4].output[0].value;
    ...
    let header_130 = BlockHeader {
        version: 0x20000000,
        prev_blockhash: header_129.block_hash(),
        merkle_root: Default::default(),
        time: 42,
        bits: 42,
        nonce: 42,
    ...
    let header_131 = BlockHeader {
        version: 0x20000000,
        prev_blockhash: header_130.block_hash(),
        merkle_root: Default::default(),
        time: 42,
        bits: 42,
        nonce: 42,
    ...
    let node_txn = {
        ...
        let fee_2 =
            revoked_htlc_txn[0].output[0].value + revoked_htlc_txn[2].output[0].value - node_txn[0].output[0].value;
...
fn test_pending_claimed_htlc_no_balance_underflow() {
    ...
    nodes[0]
        .node
        .handle_update_fulfill_htlc(&nodes[1].node.get_our_node_id(), &fulfill_ev.update_fulfill_htlcs[0]);
    expect_payment_sent_without_paths!(nodes[0], payment_preimage);
    nodes[0]
        .node
        .handle_commitment_signed(&nodes[1].node.get_our_node_id(), &fulfill_ev.commitment_signed);
    check_added_monitors!(nodes[0], 1);
...
fn test_channel_conf_timeout() {
    ...
    let _funding_tx = create_chan_between_nodes_with_value_init(
        &nodes[0],
        &nodes[1],
        1_000_000,
        100_000,
...
fn test_override_0msat_htlc_minimum() {
    ...
    nodes[1]
        .node
        .handle_open_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &res);
    let res = get_event_msg!(
        nodes[1],
        MessageSendEvent::SendAcceptChannel,
        nodes[0].node.get_our_node_id()
...
fn test_preimage_storage() {
    ...
    assert_eq!(events.len(), 1);
    match events[0] {
        Event::PaymentReceived { ref purpose, .. } => match &purpose {
...
fn test_secret_timeout() {
    ...
    if let Err(APIError::APIMisuseError { err }) =
        nodes[1]
            .node
            .create_inbound_payment_for_hash_legacy(payment_hash, Some(100_000), 2)
    {
    ...
    let mut block = {
        ...
        Block {
            header: BlockHeader {
                ...
                merkle_root: Default::default(),
                time: node_1_blocks.len() as u32 + 7200,
                bits: 42,
                nonce: 42,
            },
            txdata: vec![],
...

    {
        ...
        nodes[0]
            .node
            .send_payment(&route, payment_hash, &Some(our_payment_secret))
            .unwrap();
        check_added_monitors!(nodes[0], 1);
        let mut events = nodes[0].node.get_and_clear_pending_msg_events();
        let mut payment_event = SendEvent::from_event(events.pop().unwrap());
...
fn test_bad_secret_hash() {
    ...
    nodes[0]
        .node
        .send_payment(&route, our_payment_hash, &Some(random_payment_secret))
        .unwrap();
    ...
    nodes[0]
        .node
        .send_payment(&route, random_payment_hash, &Some(our_payment_secret))
        .unwrap();
    handle_unknown_invalid_payment_data!();
    expect_payment_failed!(
        nodes[0],
    ...
    nodes[0]
        .node
        .send_payment(&route, random_payment_hash, &Some(random_payment_secret))
        .unwrap();
    handle_unknown_invalid_payment_data!();
    expect_payment_failed!(
        nodes[0],
...
fn test_update_err_monitor_lockdown() {
    ...
    let watchtower = {
        ...
        let new_monitor = <(BlockHash, channelmonitor::ChannelMonitor<EnforcingSigner>)>::read(
            ...
            &test_utils::OnlyReadsKeysInterface {},
        )
        .unwrap()
        .1;
    ...
    watchtower
        .chain_monitor
        .block_connected(&Block { header, txdata: vec![] }, 200);
    ...
    if let Some(ref mut channel) = nodes[0].node.channel_state.lock().unwrap().by_id.get_mut(&chan_1.2) {
        if let Ok((_, _, update)) = channel.commitment_signed(&updates.commitment_signed, &node_cfgs[0].logger) {
            if let Err(_) = watchtower.chain_monitor.update_channel(outpoint, update.clone()) {
            } else {
                assert!(false);
            }
            if let Ok(_) = nodes[0].chain_monitor.update_channel(outpoint, update) {
            } else {
                assert!(false);
            }
        } else {
...
fn test_concurrent_monitor_claim() {
    ...
    // Make the tx_broadcaster aware of enough blocks that it doesn't think we're violating
    // transaction lock time requirements here.
    chanmon_cfgs[0].tx_broadcaster.blocks.lock().unwrap().resize(
        (CHAN_CONFIRM_DEPTH + 1 + TEST_FINAL_CLTV + LATENCY_GRACE_PERIOD_BLOCKS) as usize,
        (header, 0),
    );
    ...
    watchtower_bob.chain_monitor.block_connected(
        &Block { header, txdata: vec![] },
        CHAN_CONFIRM_DEPTH + TEST_FINAL_CLTV + LATENCY_GRACE_PERIOD_BLOCKS,
    );
    ...
    if let Some(ref mut channel) = nodes[0].node.channel_state.lock().unwrap().by_id.get_mut(&chan_1.2) {
        if let Ok((_, _, update)) = channel.commitment_signed(&updates.commitment_signed, &node_cfgs[0].logger) {
            ...
            if let Err(_) = watchtower_alice.chain_monitor.update_channel(outpoint, update.clone()) {
            } else {
                assert!(false);
            }
            if let Ok(_) = watchtower_bob.chain_monitor.update_channel(outpoint, update.clone()) {
            } else {
                assert!(false);
            }
            if let Ok(_) = nodes[0].chain_monitor.update_channel(outpoint, update) {
    ...
    watchtower_alice.chain_monitor.block_connected(
        &Block {
            header,
            txdata: vec![bob_state_y.clone()],
        },
        CHAN_CONFIRM_DEPTH + 2 + TEST_FINAL_CLTV + LATENCY_GRACE_PERIOD_BLOCKS,
    );
    {
...
fn test_pre_lockin_no_chan_closed_update() {
    ...
    nodes[1]
        .node
        .handle_open_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &open_chan_msg);
    ...
    let channel_id = ::chain::transaction::OutPoint {
...
fn do_test_onchain_htlc_settlement_after_close(broadcast_alice: bool, go_onchain_before_fulfill: bool) {
    ...
    nodes[1]
        .node
        .handle_update_fulfill_htlc(&nodes[2].node.get_our_node_id(), &carol_updates.update_fulfill_htlcs[0]);
    expect_payment_forwarded!(
        nodes[1],
        if go_onchain_before_fulfill || force_closing_node == 1 {
            None
        } else {
            Some(1000)
        },
    ...
    nodes[1]
        .node
        .handle_commitment_signed(&nodes[2].node.get_our_node_id(), &carol_updates.commitment_signed);
...
fn test_duplicate_chan_id() {
    ...
    let funding_created = {
        ...
        let mut as_chan = a_channel_lock
            .by_id
            .get_mut(&open_chan_2_msg.temporary_channel_id)
            .unwrap();
    ...
    nodes[0]
        .node
        .handle_funding_signed(&nodes[1].node.get_our_node_id(), &funding_signed_msg);
    {
        let mut added_monitors = nodes[0].chain_monitor.added_monitors.lock().unwrap();
        assert_eq!(added_monitors.len(), 1);
        assert_eq!(added_monitors[0].0, funding_output);
...
fn test_error_chans_closed() {
    ...
    let chan_3 = create_announced_chan_between_nodes_with_value(
        &nodes,
        0,
        2,
        100000,
    ...
    let _chan_4 = create_announced_chan_between_nodes_with_value(
        &nodes,
        0,
        1,
        100000,
...
fn test_invalid_funding_tx() {
    ...
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), 100_000, 10_000, 42, None)
        .unwrap();
    nodes[1].node.handle_open_channel(
        &nodes[0].node.get_our_node_id(),
        InitFeatures::known(),
    ...
    nodes[0]
        .node
        .funding_transaction_generated_unchecked(&temporary_channel_id, tx.clone(), 0)
        .unwrap();
...
fn do_test_tx_confirmed_skipping_blocks_immediate_broadcast(test_height_before_timelock: bool) {
    ...
    *nodes[0].connect_style.borrow_mut() = ConnectStyle::BestBlockFirstSkippingBlocks;
    ...
    create_announced_chan_between_nodes(&nodes, 0, 1, InitFeatures::known(), InitFeatures::known());
    let (chan_announce, _, channel_id, _) =
        create_announced_chan_between_nodes(&nodes, 1, 2, InitFeatures::known(), InitFeatures::known());
...
fn test_forwardable_regen() {
    ...
    let persister: test_utils::TestPersister;
    let new_chain_monitor: test_utils::TestChainMonitor;
    let nodes_1_deserialized: ChannelManager<
        EnforcingSigner,
        &test_utils::TestChainMonitor,
        &test_utils::TestBroadcaster,
        &test_utils::TestKeysInterface,
    ...
    get_monitor!(nodes[1], chan_id_1)
        .write(&mut chan_0_monitor_serialized)
        .unwrap();
    get_monitor!(nodes[1], chan_id_2)
        .write(&mut chan_1_monitor_serialized)
        .unwrap();
    ...
    new_chain_monitor = test_utils::TestChainMonitor::new(
        ...
        nodes[1].tx_broadcaster.clone(),
        nodes[1].logger,
        node_cfgs[1].fee_estimator,
        &persister,
        keys_manager,
    );
    ...
    let (_, mut chan_1_monitor) =
        <(BlockHash, ChannelMonitor<EnforcingSigner>)>::read(&mut chan_1_monitor_read, keys_manager).unwrap();
    ...
    assert!(nodes[1]
        .chain_monitor
        .watch_channel(chan_1_monitor.get_funding_txo().0, chan_1_monitor)
        .is_ok());
...
fn test_dup_htlc_second_fail_panic() {
    ...
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &fail_updates_1.update_fail_htlcs[0]);
    nodes[0]
        .node
        .handle_update_fail_htlc(&nodes[1].node.get_our_node_id(), &fail_updates_1.update_fail_htlcs[1]);
    ...
    if let Event::PaymentPathFailed { .. } = failure_events[1] {
    } else {
        panic!();
    }
}
...
fn test_keysend_payments_to_public_node() {
    ...
    let (payment_hash, _) = nodes[0]
        .node
        .send_spontaneous_payment(&route, Some(test_preimage))
        .unwrap();
    check_added_monitors!(nodes[0], 1);
    let mut events = nodes[0].node.get_and_clear_pending_msg_events();
    assert_eq!(events.len(), 1);
...
fn do_test_max_dust_htlc_exposure(
    ...
    nodes[0]
        .node
        .create_channel(nodes[1].node.get_our_node_id(), 1_000_000, 500_000_000, 42, None)
        .unwrap();
    let mut open_channel = get_event_msg!(
        nodes[0],
        MessageSendEvent::SendOpenChannel,
    ...
    let dust_outbound_htlc_on_holder_tx_msat: u64 =
        (dust_buffer_feerate * htlc_timeout_tx_weight(opt_anchors) / 1000 + open_channel.dust_limit_satoshis - 1)
            * 1000;
    let dust_outbound_htlc_on_holder_tx: u64 =
        config.channel_options.max_dust_htlc_exposure_msat / dust_outbound_htlc_on_holder_tx_msat;

    let dust_inbound_htlc_on_holder_tx_msat: u64 =
        (dust_buffer_feerate * htlc_success_tx_weight(opt_anchors) / 1000 + open_channel.dust_limit_satoshis - 1)
            * 1000;
    let dust_inbound_htlc_on_holder_tx: u64 =
        config.channel_options.max_dust_htlc_exposure_msat / dust_inbound_htlc_on_holder_tx_msat;

    let dust_htlc_on_counterparty_tx: u64 = 25;
    let dust_htlc_on_counterparty_tx_msat: u64 =
        config.channel_options.max_dust_htlc_exposure_msat / dust_htlc_on_counterparty_tx;
    ...
    if on_holder_tx {
        if dust_outbound_balance {
            ...
            for i in 0..dust_outbound_htlc_on_holder_tx {
                let (route, payment_hash, _, payment_secret) =
                    get_route_and_payment_hash!(nodes[0], nodes[1], dust_outbound_htlc_on_holder_tx_msat);
                if let Err(_) = nodes[0].node.send_payment(&route, payment_hash, &Some(payment_secret)) {
                    panic!("Unexpected event at dust HTLC {}", i);
                }
            }
        } else {
        ...
        if dust_outbound_balance {
            ...
            for i in 0..dust_htlc_on_counterparty_tx {
                let (route, payment_hash, _, payment_secret) =
                    get_route_and_payment_hash!(nodes[0], nodes[1], dust_htlc_on_counterparty_tx_msat);
                if let Err(_) = nodes[0].node.send_payment(&route, payment_hash, &Some(payment_secret)) {
                    panic!("Unexpected event at dust HTLC {}", i);
                }
            }
    ...
    if exposure_breach_event == ExposureEvent::AtHTLCForward {
        ...
        if on_holder_tx {
            ...
            let dust_inbound_overflow = dust_inbound_htlc_on_holder_tx_msat * dust_inbound_htlc_on_holder_tx
                + dust_outbound_htlc_on_holder_tx_msat;
        ...
        if on_holder_tx {
            ...
            let dust_outbound_overflow = dust_outbound_htlc_on_holder_tx_msat * dust_outbound_htlc_on_holder_tx
                + dust_inbound_htlc_on_holder_tx_msat;
        ...
        if let Err(_) = nodes[0].node.send_payment(&route, payment_hash, &Some(payment_secret)) {
            panic!("Unexpected event at update_fee-swallowed HTLC",);
        }
        {
            let mut feerate_lock = chanmon_cfgs[0].fee_estimator.sat_per_kw.lock().unwrap();
        ...
        nodes[0].logger.assert_log_contains(
            "lightning::ln::channel".to_string(),
            "Cannot afford to send new feerate at 2530 without infringing max dust htlc exposure".to_string(),
            1,
        );
    }
