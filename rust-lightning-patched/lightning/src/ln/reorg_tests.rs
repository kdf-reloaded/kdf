...
fn do_test_onchain_htlc_reorg(local_commitment: bool, claim: bool) {
    ...
    let mut header = BlockHeader {
        version: 0x2000_0000,
        prev_blockhash: nodes[2].best_block_hash(),
    ...
    let claim_txn = if local_commitment {
        ...
        connect_block(
            ...
            &Block {
                header,
                txdata: node_1_commitment_txn.clone(),
            },
        );
        check_added_monitors!(nodes[2], 1);
...
fn do_test_unconf_chan(
    reload_node: bool,
    reorg_after_reload: bool,
    use_funding_unconfirmed: bool,
    ...
    *nodes[0].chain_monitor.expect_channel_force_closed.lock().unwrap() = Some((chan.2, true));
    ...
    check_closed_event!(
        ...
        ClosureReason::CounterpartyForceClosed {
            peer_msg: "Channel closed because of an exception: ".to_owned() + expected_err
        }
    );
    check_closed_event!(
        nodes[0],
