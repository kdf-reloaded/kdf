...
use chain::{keysinterface::KeysInterface, BestBlock, Confirm, Listen, Watch};
use ln::channelmanager::{
    ChainParameters, ChannelManager, ChannelManagerReadArgs, PaymentId, PaymentSendFailure, RAACommitmentOrder,
...
use routing::router::{get_route, PaymentParameters, Route};
    ...
    for i in 0..count {
        ...
        match *node.connect_style.borrow() {
            ConnectStyle::FullBlockViaListen => {
                node.chain_monitor
                    .chain_monitor
                    .block_disconnected(&orig_header.0, orig_header.1);
            ...
            ConnectStyle::BestBlockFirstSkippingBlocks | ConnectStyle::TransactionsFirstSkippingBlocks => {
                if i == count - 1 {
                    node.chain_monitor
                        .chain_monitor
                        .best_block_updated(&prev_header.0, prev_header.1);
...
pub struct Node<'a, 'b: 'a, 'c: 'b> {
    ...
    pub net_graph_msg_handler:
        NetGraphMsgHandler<&'c NetworkGraph, &'c test_utils::TestChainSource, &'c test_utils::TestLogger>,
    pub node_seed: [u8; 32],
        ...
        if !panicking() {
            ...
            // Check that if we serialize the Router, we can deserialize it again.
            {
                ...
                let net_graph_msg_handler =
                    NetGraphMsgHandler::new(&network_graph_deser, Some(self.chain_source), self.logger);
                let mut chan_progress = 0;
                loop {
                    let orig_announcements = self
                        .net_graph_msg_handler
                        .get_next_channel_announcements(chan_progress, 255);
                    let deserialized_announcements =
                        net_graph_msg_handler.get_next_channel_announcements(chan_progress, 255);
                ...
                loop {
                    let orig_announcements = self
                        .net_graph_msg_handler
                        .get_next_node_announcements(node_progress.as_ref(), 255);
                    let deserialized_announcements =
                        net_graph_msg_handler.get_next_node_announcements(node_progress.as_ref(), 255);
                    assert!(orig_announcements == deserialized_announcements);
            ...
            let feeest = test_utils::TestFeeEstimator {
            ...
            for deserialized_monitor in deserialized_monitors.drain(..) {
                if let Err(_) =
                    chain_monitor.watch_channel(deserialized_monitor.get_funding_txo().0, deserialized_monitor)
                {
                    panic!();
                }
            }
            assert_eq!(
                *chain_source.watched_txn.lock().unwrap(),
                *self.chain_source.watched_txn.lock().unwrap()
            );
            assert_eq!(
                *chain_source.watched_outputs.lock().unwrap(),
                *self.chain_source.watched_outputs.lock().unwrap()
            );
        }
    }
}
    ...
    ($node: expr, $node_id: expr) => {{
        ...
        match events[0] {
            $crate::util::events::MessageSendEvent::HandleError {
                action: $crate::ln::msgs::ErrorAction::SendErrorMessage { ref msg },
                ref node_id,
            } => {
                assert_eq!(*node_id, $node_id);
                (*msg).clone()
...
macro_rules! get_htlc_update_msgs {
    ($node: expr, $node_id: expr) => {{
        ...
        assert_eq!(events.len(), 1);
        match events[0] {
            $crate::util::events::MessageSendEvent::UpdateHTLCs {
                ref node_id,
                ref updates,
            } => {
                assert_eq!(*node_id, $node_id);
    ...
    ($node: expr, $channel_id: expr) => {{
        ...
        for index in 0..2 {
            if let Ok(mon) = $node
                .chain_monitor
                .chain_monitor
                .get_monitor($crate::chain::transaction::OutPoint {
                    txid: bitcoin::Txid::from_slice(&$channel_id[..]).unwrap(),
                    index,
                })
            {
                monitor = Some(mon);
...
pub fn sign_funding_transaction<'a, 'b, 'c>(
    ...
    node_b: &Node<'a, 'b, 'c>,
    channel_value: u64,
    expected_temporary_channel_id: [u8; 32],
) -> Transaction {
    ...
    node_b
        .node
        .handle_funding_created(&node_a.node.get_our_node_id(), &funding_created_msg);
    {
...
pub fn create_chan_between_nodes_with_value_init<'a, 'b, 'c>(
    ...
    node_b
        .node
        .handle_open_channel(&node_a.node.get_our_node_id(), a_flags, &open_channel_msg);
    ...
    node_a
        .node
        .handle_accept_channel(&node_b.node.get_our_node_id(), b_flags, &accept_channel_msg);
...
pub fn create_unannounced_chan_between_nodes_with_value<'a, 'b, 'c, 'd>(
    ...
    a_flags: InitFeatures,
    b_flags: InitFeatures,
) -> (msgs::FundingLocked, Transaction) {
    ...
    nodes[b]
        .node
        .handle_open_channel(&nodes[a].node.get_our_node_id(), a_flags, &open_channel);
    ...
    nodes[a]
        .node
        .handle_accept_channel(&nodes[b].node.get_our_node_id(), b_flags, &accept_channel);
    ...
    nodes[a]
        .node
        .handle_funding_signed(&nodes[b].node.get_our_node_id(), &cs_funding_signed);
    ...
    nodes[a].node.handle_funding_locked(
        &nodes[b].node.get_our_node_id(),
        &get_event_msg!(
            nodes[b],
            MessageSendEvent::SendFundingLocked,
    ...
    nodes[b]
        .node
        .handle_funding_locked(&nodes[a].node.get_our_node_id(), &as_funding_locked);
    ...
    nodes[a]
        .node
        .handle_channel_update(&nodes[b].node.get_our_node_id(), &bs_update);
    nodes[b]
        .node
        .handle_channel_update(&nodes[a].node.get_our_node_id(), &as_update);
...
pub fn update_nodes_with_chan_announce<'a, 'b, 'c, 'd>(
    ...
    for node in nodes {
        ...
        node.node
            .handle_channel_update(&nodes[a].node.get_our_node_id(), &upd_1);
        node.node
            .handle_channel_update(&nodes[b].node.get_our_node_id(), &upd_2);
    }
}
...
macro_rules! get_payment_preimage_hash {
    ...
    ($dest_node: expr, $min_value_msat: expr) => {{
        ...
        *payment_count += 1;
        let payment_hash =
            $crate::ln::PaymentHash(bitcoin::hashes::sha256::Hash::hash(&payment_preimage.0[..]).into_inner());
        let payment_secret = $dest_node
            .node
            .create_inbound_payment_for_hash(payment_hash, $min_value_msat, 7200)
            .unwrap();
...
macro_rules! get_route_and_payment_hash {
    ...
    ($send_node: expr, $recv_node: expr, $last_hops: expr, $recv_value: expr, $cltv: expr) => {{
        ...
        let (payment_preimage, payment_hash, payment_secret) =
            $crate::get_payment_preimage_hash!($recv_node, Some($recv_value));
        let payment_params =
            $crate::routing::router::PaymentParameters::from_node_id($recv_node.node.get_our_node_id())
                .with_features($crate::ln::features::InvoiceFeatures::known())
                .with_route_hints($last_hops);
        let scorer = $crate::util::test_utils::TestScorer::with_penalty(0);
        let keys_manager =
            $crate::util::test_utils::TestKeysInterface::new(&[0u8; 32], bitcoin::network::constants::Network::Testnet);
    ...
    ($node: expr, $expected_payment_hash: expr, $expected_payment_secret: expr, $expected_recv_value: expr, $expected_payment_preimage: expr) => {
        ...
        match events[0] {
            $crate::util::events::Event::PaymentReceived {
                ...
                match purpose {
                    $crate::util::events::PaymentPurpose::InvoicePayment {
                        payment_preimage,
                        payment_secret,
                        ..
                    } => {
...
macro_rules! expect_payment_sent {
    ...
    ($node: expr, $expected_payment_preimage: expr, $expected_fee_msat_opt: expr, $expect_paths: expr) => {{
        ...
        let expected_payment_hash =
            $crate::ln::PaymentHash(bitcoin::hashes::sha256::Hash::hash(&$expected_payment_preimage.0).into_inner());
        if $expect_paths {
            assert!(events.len() > 1);
        } else {
            assert_eq!(events.len(), 1);
        ...
        let expected_payment_id = match events[0] {
            $crate::util::events::Event::PaymentSent {
                ref payment_id,
    ...
    ($node: expr, $expected_payment_hash: expr, $rejected_by_dest: expr, $conditions: expr) => {
        ...
        let expected_payment_id = match events[0] {
            Event::PaymentPathFailed {
                ...
                assert_eq!(
                    retry.as_ref().unwrap().final_value_msat,
                    path.last().unwrap().fee_msat,
                    "Retry amount should match last hop in path"
                );
                assert_eq!(
                    retry.as_ref().unwrap().payment_params.payee_pubkey,
                    path.last().unwrap().pubkey,
                    "Retry payee node_id should match last hop in path"
                );
                ...
                if let Some(chan_closed) = $conditions.expected_blamed_chan_closed {
                    match network_update {
                        &Some($crate::routing::network_graph::NetworkUpdate::ChannelUpdateMessage { ref msg })
                            if !chan_closed =>
                        {
                        ...
                        &Some($crate::routing::network_graph::NetworkUpdate::ChannelClosed {
                            short_channel_id,
                            is_permanent,
                        }) if chan_closed => {
                            if let Some(scid) = $conditions.expected_blamed_scid {
    ...
    for (expected_route, (path_msgs, next_hop)) in expected_paths.iter().zip(per_path_msgs.drain(..)) {
        ...
        macro_rules! last_update_fulfill_dance {
            ($node: expr, $prev_node: expr) => {{
                $node
                    .node
                    .handle_update_fulfill_htlc(&$prev_node.node.get_our_node_id(), &next_msgs.as_ref().unwrap().0);
                check_added_monitors!($node, 0);
...
pub fn fail_payment_along_route<'a, 'b, 'c>(
    ...
    for (i, (expected_route, (path_msgs, next_hop))) in expected_paths.iter().zip(per_path_msgs.drain(..)).enumerate() {
        ...
        for (idx, node) in expected_route.iter().rev().enumerate().skip(1) {
            ...
            if next_msgs.is_some() {
                node.node
                    .handle_update_fail_htlc(&prev_node.node.get_our_node_id(), &next_msgs.as_ref().unwrap().0);
    ...
    for i in 0..node_count {
        for j in (i + 1)..node_count {
            nodes[i].node.peer_connected(
                ...
                &msgs::Init {
                    features: cfgs[j].features.clone(),
            ...
            nodes[j].node.peer_connected(
                ...
                &msgs::Init {
                    features: cfgs[i].features.clone(),
                    remote_network_address: None,
                },
            );
        }
...
pub fn test_txn_broadcast<'a, 'b, 'c>(
    node: &Node<'a, 'b, 'c>,
    chan: &(msgs::ChannelUpdate, msgs::ChannelUpdate, [u8; 32], Transaction),
...
macro_rules! handle_chan_reestablish_msgs {
    ($src_node: expr, $dst_node: expr) => {{
        ...
        let mut idx = 0;
        let funding_locked =
            if let Some(&MessageSendEvent::SendFundingLocked { ref node_id, ref msg }) = msg_events.get(0) {
                idx += 1;
                assert_eq!(*node_id, $dst_node.node.get_our_node_id());
                Some(msg.clone())
            } else {
...
pub fn reconnect_nodes<'a, 'b, 'c>(
    node_a: &Node<'a, 'b, 'c>,
    node_b: &Node<'a, 'b, 'c>,
    send_funding_locked: (bool, bool),
    pending_htlc_adds: (i64, i64),
    ...
    for msg in reestablish_1 {
        node_b
            .node
            .handle_channel_reestablish(&node_a.node.get_our_node_id(), &msg);
    ...
    for msg in reestablish_2 {
        node_a
            .node
            .handle_channel_reestablish(&node_b.node.get_our_node_id(), &msg);
    ...
    for chan_msgs in resp_1.drain(..) {
        if send_funding_locked.0 {
            node_a
                .node
                .handle_funding_locked(&node_b.node.get_our_node_id(), &chan_msgs.0.unwrap());
            ...
            || pending_cell_htlc_fails.0 != 0
        {
            ...
            for update_add in commitment_update.update_add_htlcs {
                node_a
                    .node
                    .handle_update_add_htlc(&node_b.node.get_our_node_id(), &update_add);
            }
            ...
            for update_fail in commitment_update.update_fail_htlcs {
                node_a
                    .node
                    .handle_update_fail_htlc(&node_b.node.get_our_node_id(), &update_fail);
            }
    ...
    for chan_msgs in resp_2.drain(..) {
        if send_funding_locked.1 {
            node_b
                .node
                .handle_funding_locked(&node_a.node.get_our_node_id(), &chan_msgs.0.unwrap());
            ...
            || pending_cell_htlc_fails.1 != 0
        {
            ...
            for update_add in commitment_update.update_add_htlcs {
                node_b
                    .node
                    .handle_update_add_htlc(&node_a.node.get_our_node_id(), &update_add);
            }
            for update_fulfill in commitment_update.update_fulfill_htlcs {
                node_b
                    .node
            ...
            for update_fail in commitment_update.update_fail_htlcs {
                node_b
                    .node
                    .handle_update_fail_htlc(&node_a.node.get_our_node_id(), &update_fail);
            }
