...
use chain::keysinterface::{BaseSign, InMemorySigner, Sign};
use ln::chan_utils::{
    ChannelPublicKeys, ChannelTransactionParameters, ClosingTransaction, CommitmentTransaction, HTLCOutputInCommitment,
...
impl BaseSign for EnforcingSigner {
    ...
    fn release_commitment_secret(&self, idx: u64) -> [u8; 32] {
        {
            ...
            assert!(
                idx == state.last_holder_revoked_commitment || idx == state.last_holder_revoked_commitment - 1,
    ...
    fn validate_counterparty_revocation(&self, idx: u64, _secret: &SecretKey) -> Result<(), ()> {
        let mut state = self.state.lock().unwrap();
        assert!(
            idx == state.last_counterparty_revoked_commitment || idx == state.last_counterparty_revoked_commitment - 1,
    ...
    fn unsafe_sign_holder_commitment_and_htlcs(
        ...
        Ok(self
            .inner
            .unsafe_sign_holder_commitment_and_htlcs(commitment_tx, secp_ctx)
            .unwrap())
    }
    ...
    fn sign_justice_revoked_htlc(
        ...
        Ok(self
            .inner
            .sign_justice_revoked_htlc(justice_tx, input, amount, per_commitment_key, htlc, secp_ctx)
            .unwrap())
    }
    ...
    fn sign_counterparty_htlc_transaction(
        ...
        Ok(self
            .inner
            .sign_counterparty_htlc_transaction(htlc_tx, input, amount, per_commitment_point, htlc, secp_ctx)
            .unwrap())
    }
    ...
    fn sign_closing_transaction(
        ...
        closing_tx
            .verify(self.inner.funding_outpoint().into_bitcoin_outpoint())
...
impl EnforcingSigner {
    fn verify_counterparty_commitment_tx<'a, T: secp256k1::Signing + secp256k1::Verification>(
        ...
        commitment_tx
            .verify(
                &self.inner.get_channel_parameters().as_counterparty_broadcastable(),
    ...
    fn verify_holder_commitment_tx<'a, T: secp256k1::Signing + secp256k1::Verification>(
        ...
        commitment_tx
            .verify(
                &self.inner.get_channel_parameters().as_holder_broadcastable(),
                self.inner.pubkeys(),
                self.inner.counterparty_pubkeys(),
                secp_ctx,
            )
