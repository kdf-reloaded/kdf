...
// `compact_formats` is machine-generated by rust-protobuf (see build.rs) and is
// overwritten on every build, so lint suppressions must live here on the module
// declaration rather than inside the generated file. These cover lints newer
// than the rust-protobuf 2.23.0 codegen (which emits its own, older allow list).
#[allow(
    unused_parens,
