...
impl BuildResult {
    ...
    /// Consumes this build result and returns the constructed transaction.
    pub fn into_transaction(self) -> Transaction { self.transaction }
...
impl<P: consensus::Parameters, U> Builder<'_, P, U> {
    ...
    /// Adds a non-standard P2SH input used by KDF-family atomic swaps.
    ///
    /// # Errors
    ///
    ...
    pub fn add_kdf_p2sh_input(
        ...
        self.transparent_builder.add_input(TransparentInputInfo::from_kdf_p2sh_parts(
            utxo,
            coin,
            pubkey,
            redeem_script,
    ...
    pub fn add_transparent_output_raw(&mut self, output: TxOut) {
        self.transparent_builder.add_output_raw(output)
    }

    /// Sets the transaction lock time.
    pub fn set_lock_time(&mut self, lock_time: u32) { self.lock_time = lock_time }
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "transparent-inputs")]
    fn kdf_builder_preserves_lock_time_and_raw_output_order() {
        use super::{BuildConfig, Builder};
        use ::transparent::{
            address::{Script, TransparentAddress},
            builder::TransparentSigningSet,
            bundle::OutPoint as TransparentOutPoint,
        };
        use secp256k1::SecretKey;
        use std::vec::Vec;
        use zcash_protocol::{consensus::NetworkUpgrade, value::Zatoshis};
        use zcash_script::script::{Code, FromChain, Sig};
        ...
        // HASH160([OP_TRUE]); fixed here so the test remains independent of
        // private helper APIs in `zcash_transparent`.
        let p2sh = TransparentAddress::ScriptHash([
        ...
        builder
            .add_kdf_p2sh_input(
                pubkey,
                TransparentOutPoint::new([1u8; 32], 1),
                prev_coin,
                redeem_script,
                script_data,
                0xffff_fffe,
        ...
        let second = TxOut::new(
            ...
            TransparentAddress::PublicKeyHash([4; 20]).script().into(),
        );
        builder.add_transparent_output_raw(first.clone());
        builder.add_transparent_output_raw(second.clone());
        ...
        assert_eq!(tx.transparent_bundle().unwrap().vin[0].sequence(), 0xffff_fffe);
        assert_eq!(tx.transparent_bundle().unwrap().vout, vec![first, second]);
        let mut encoded = Vec::new();
        tx.write(&mut encoded).unwrap();
        assert_eq!(
            hex::encode(encoded),
            ""
        );
    }
