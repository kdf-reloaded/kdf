...
pub trait VerifyableClaims: Serialize + DeserializeOwned {
        ...
        Self: Sized,
    {
        ...
        // PATCHED: upstream verified the Ed25519 signature through
        // `jsonwebtoken::crypto::verify`, which is the only thing that crate was
        // used for here -- claim validation is hand-rolled in `verify_basic`
        // below. That pulled in jsonwebtoken 8.3.0 (GHSA-h395-gr6q-cpjc, type
        // confusion in *its* claim validation) and, transitively, ring 0.16.20.
        // Both are gone now: verification uses `ed25519_dalek`, which this file
        // already uses to produce the signature in `encode` above, so signing and
        // verifying are symmetric and no extra dependency is needed.
        //
        // `verify_strict` rather than `verify`: it rejects small-order and
        // non-canonical public keys, matching what ring did underneath
        // jsonwebtoken, and is the stricter of the two.
        let public_key = claims.basic().iss.0.as_public_key();

        // `JwtError::Signature`, not `Encoding`, for a signature that will not
        // even base64-decode: upstream's `jsonwebtoken::crypto::verify` decoded
        // internally and any failure fell through relay_rpc's `_ =>
        // Err(JwtError::Signature)` arm. Keeping that mapping keeps the error
        // surface identical -- the `token_validation` test below pins it, with
        // an 87-character signature that is not a valid base64url-nopad length.
        let signature_bytes = data_encoding::BASE64URL_NOPAD
