...
pub enum SpendInfo {
    ...
    /// A non-standard P2SH spend used by KDF-family atomic swaps.
    ///
    /// The script signature is assembled as `<signature> <script_data> <redeem_script>`,
    /// matching the legacy KDF Zcash transaction contract.
    KdfP2sh {
        pubkey: secp256k1::PublicKey,
        redeem_script: script::FromChain,
        script_data: script::Sig,
    },
}
...
impl TransparentInputInfo {
    ...
    pub fn from_parts(utxo: OutPoint, coin: TxOut, spend_info: SpendInfo) -> Result<Self, Error> {
        match &spend_info {
            ...
            SpendInfo::P2sh { redeem_script } | SpendInfo::KdfP2sh { redeem_script, .. } => {
                // Ensure that the RIPEMD-160 digest of the public key associated with the
                // provided secret key matches that of the address to which the provided
                // output may be spent.
    ...
    /// Constructs a non-standard P2SH input used by KDF-family atomic swaps.
    ///
    /// # Errors
    ///
    /// Returns an error if the redeem script does not match the spent output
    /// or if its serialized input size cannot be represented.
    pub fn from_kdf_p2sh_parts(
        utxo: OutPoint,
        coin: TxOut,
        pubkey: secp256k1::PublicKey,
        redeem_script: script::FromChain,
    ...
    pub fn serialized_len(&self) -> Option<usize> {
        match &self.spend_info {
            ...
            SpendInfo::KdfP2sh {
                ...
                let fake_sig = pv::push_value(&[0; MAX_SIG_SIZE])?;
                let mut script_sig = vec![fake_sig];
                script_sig.extend(script_data.0.iter().cloned());
                script_sig.push(push_script(redeem_script)?);
                let script_len = script::Component(script_sig).byte_len();
                Some(PREVOUT_SIZE + CompactSize::serialized_size(script_len) + script_len + SEQUENCE_SIZE)
            }
        }
    }
}
...
impl TransparentBuilder {
    ...
    /// Adds an already-constructed transparent output.
    pub fn add_output_raw(&mut self, output: TxOut) { self.vout.push(output) }
    ...
    pub fn build(self) -> Option<Bundle<Unauthorized>> {
        ...
        let vin: Vec<TxIn<Unauthorized>> = self
            .inputs
            .iter()
            .map(|i| TxIn::from_parts(i.utxo.clone(), (), i.sequence))
            .collect();
            ...
            .map(|(index, info)| {
                match &info.spend_info {
                    ...
                    SpendInfo::KdfP2sh {
                        ...
                        let mut script_sig = vec![pv::push_value(&sig_bytes).ok_or(Error::UnsupportedScript)?];
                        script_sig.extend(script_data.0.iter().cloned());
                        script_sig.push(push_script(redeem_script).ok_or(Error::UnsupportedScript)?);
                        Ok(script::Component(script_sig))
                    }
                }
            });
...
mod tests {
    ...
    use super::{Error, OutPoint, SignableInput, TransparentBuilder, TransparentInputInfo, TxOut};
    ...
    #[test]
    fn kdf_p2sh_preserves_sequence_and_script_component_order() {
        use super::TransparentSigningSet;
        use zcash_script::script::{Code, Evaluable, FromChain, Sig};

        let sk = SecretKey::from_slice(&[1; 32]).unwrap();
        let pk = secp256k1::PublicKey::from_secret_key(&Secp256k1::new(), &sk);
        let redeem_script = FromChain::parse(&Code(vec![0x51])).unwrap();
        let script_data = Sig::parse(&Code(vec![0x51])).unwrap();
        let p2sh = TransparentAddress::ScriptHash(crate::util::hash160::hash(&redeem_script.to_bytes()));
        let coin = TxOut::new(Zatoshis::from_u64(10_000).unwrap(), p2sh.script().into());
        let sequence = 0xffff_fffe;
        let input = TransparentInputInfo::from_kdf_p2sh_parts(
            OutPoint::new([7; 32], 3),
            coin,
            pk,
            redeem_script,
        ...
        let signed = bundle
            .apply_signatures(|_| [42; 32], &signing_set)
            .unwrap();

        assert_eq!(signed.vin[0].sequence(), sequence);
        let script_sig = &signed.vin[0].script_sig().0.0;
        let signature_len = usize::from(script_sig[0]);
        assert_eq!(script_sig[signature_len], 0x01, "signature must use SIGHASH_ALL");
        assert_eq!(
            &script_sig[signature_len + 1..],
            &[0x51, 0x01, 0x51],
            "scriptSig must be <signature> <script_data> <redeem_script>"
        );
    }

    #[test]
    fn raw_outputs_preserve_count_order_value_and_script() {
        use crate::address::Script;
        use zcash_script::script::Code;

        let first = TxOut::new(
            Zatoshis::from_u64(123).unwrap(),
            Script(Code(vec![0x6a, 0x01, 0x2a])),
        );
        let second = TxOut::new(
            Zatoshis::from_u64(456).unwrap(),
            Script(Code(vec![0x51])),
        );
        let mut builder = TransparentBuilder::empty();
        builder.add_output_raw(first.clone());
        builder.add_output_raw(second.clone());

        let bundle = builder.build().unwrap();
        assert_eq!(bundle.vout, vec![first, second]);
    }
