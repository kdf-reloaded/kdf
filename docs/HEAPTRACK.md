...
```
heaptrack ./kdf "{\"gui\":\"MM2GUI\",\"netid\":8762, \"userhome\":\"/${HOME#\"/\"}\", \"passphrase\":\"YOUR_PASSPHRASE_HERE\", \"rpc_password\":\"YOUR_PASSWORD_HERE\",\"i_am_seed\":true}" &
```
...
4. After running MM2 for sometime we can visualize the memory profiling results using the below command. Note that ```heaptrack.kdf.xxxx.gz``` is the name of the file generated through the above command with numbers instead of xxxx
```
