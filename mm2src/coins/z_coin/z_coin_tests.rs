...
fn zombie_coin_validate_dex_fee() {
    ...
    let tx = ZTransaction::read(tx_bytes.as_slice(), BranchId::Sapling).unwrap();
    let tx = tx.into();
    let invalid_amount_fee = DexFee::Standard("0.001".parse::<BigDecimal>().unwrap().into());
    let valid_fee = DexFee::Standard("0.01".parse::<BigDecimal>().unwrap().into());
    let valid_uuid = [1; 16];
    let invalid_uuid = [2; 16];
